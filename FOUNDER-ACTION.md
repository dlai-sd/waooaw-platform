...
| **T1-2b** | FA-024 | **Google OAuth Client ID + Secret (user login)** — same GCP project `waooaw-platform` → APIs & Services → Credentials → Create Credentials → OAuth 2.0 Client ID → Application type: Web Application → Name: `WAOOAW Portal` → Authorised redirect URIs: `https://auth.waooaw.com/realms/waooaw/broker/google/endpoint` (prod) + `http://localhost:8080/realms/waooaw/broker/google/endpoint` (dev) → Create → copy Client ID + Client Secret. **NOTE: This is DIFFERENT from the Vertex AI SA key (FA-021) — that key is for AI inference. This key is for "Continue with Google" user login.** | `GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` | Azure Key Vault (dev) → `GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` | WC-016-02 (Registration + Auth flow) — Keycloak reads these from Key Vault and configures the Google IDP. Without this, "Continue with Google" login will fail. | 5 minutes (same GCP project as FA-021) | ⬜ PENDING |
