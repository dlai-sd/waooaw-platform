...
def _extract_dotnet_patterns(cs_content: str, file_path: str, sprint: str) -> list[CanonicalPattern]:
    ...
    di_match = re.search(r"public static IServiceCollection (Add\w+Services)\s*\([^)]*\)", cs_content)
    if di_match:
        method_name = di_match.group(1)
        # Extract the full method body if available
        body_match = re.search(rf"{re.escape(method_name)}\s*\([^)]*\)[^{{]*\{{([^}}]+)\}}", cs_content, re.DOTALL)
    ...
    grpc_method = re.search(r"public override async Task<(\w+Response)> (\w+)\s*\(\s*(\w+Request)", cs_content)
    if grpc_method:
...
def _write_patterns(patterns: list[CanonicalPattern], sprint: str) -> int:
    ...
    for stack, stack_patterns in by_stack.items():
        ...
        content_lines = [
            ...
            f"**Extracted:** {datetime.now(timezone.utc).date().isoformat()}  \n",
...
def seed_from_sprint(sprint_id: str) -> int:
    ...
    try:
        result = subprocess.run(
            ["git", "log", "--name-only", "--oneline", "-20", "--", "src/*", "tests/*"],  # noqa: S607
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
        all_files = [
            ...
            if line.strip() and not line.startswith(tuple("0123456789abcdef")) and (line.endswith(".cs") or line.endswith(".py"))
        ]
    except Exception:
        # Fallback: scan src/ directly
        all_files = [str(f.relative_to(REPO_ROOT)) for f in (REPO_ROOT / "src").rglob("*.cs")] + [
    ...
    print("  Next: Constitutional Analyst review → promote to CANONICAL")
    return written
...
def main() -> int:
    ...
    if not args:
        ...
        result = subprocess.run(["git", "log", "--oneline", "-5"], capture_output=True, text=True, cwd=REPO_ROOT)  # noqa: S607
