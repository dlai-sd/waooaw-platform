    ...
    def _select_model(self, category: TaskCategory, request: "MagicLLMRequest" = None) -> tuple[str, float]:
        ...
        if category in (TaskCategory.CODE_GENERATION, TaskCategory.TEST_GENERATION,
            ...
            if request is not None:
                ...
                complexity = _task_complexity_score(request)
                # WC035-03 context-pressure gate REMOVED — caused all WBE tasks to route to Sonnet
                # (WBE contexts are 42-45k chars by design). Under UDCP, logic-only prompts will
                # be ~3-8k chars so this gate is unnecessary and harmful.
                # Only retain retry-exhaustion upgrade (2+ failures on non-trivial context).
