...
def _call_llm_direct(
    ...
    try:
        ...
        if use_thinking:
            ...
            print(
                f"  Thinking: enabled | budget={THINKING_BUDGET} | effective_max={effective_max_tokens} "
        ...
        print(
            f"  REQ:  {task_id} attempt={attempt} | prompt={prompt_chars:,} chars | max_tokens={effective_max_tokens} (code={max_tokens})"
        )
        ...
        response = urllib.request.urlopen(req, timeout=api_timeout)  # noqa: S310
        ...
        print(
            f"  LLM:  {task_id} attempt={attempt} → {tokens_in} in / {tokens_out} out | "
...
def call_llm_for_udcp(
    ...
    print(f"  UDCP REQ: {task_id} attempt={attempt} model={model_id} prompt={prompt_chars:,} chars max_tokens={max_tokens}")
    ...
    t0 = _time.monotonic()
    try:
        response = _urlreq.urlopen(req, timeout=timeout)  # noqa: S310
        with response as resp:
    ...
    print(f"  UDCP RSP: {task_id} latency={latency:.1f}s in={in_tok} out={out_tok} chars={len(text)}")
    ...
    _MONITOR_SIGNAL["file_costs"][task_id] = _MONITOR_SIGNAL["file_costs"].get(task_id, 0.0) + round(
        (in_tok / 1000) * _r_in + (out_tok / 1000) * _r_out, 4
    )
    return text or None
...
def call_llm_via_magiclm(
    ...
    try:
        ...
        try:
            ...
            return _call_llm_direct(
                task_id, task_description, spec_content, constitutional_check, model_hint, max_tokens, attempt
            )
    ...
    try:
        ...
        return _call_llm_direct(task_id, task_description, spec_content, constitutional_check, model_hint, max_tokens, attempt)

    if response.status == "accepted":
        print(
            f"  ✓ MagicLLM: {response.model_version} · "
        ...
        _MONITOR_SIGNAL["file_costs"][task_id] = _MONITOR_SIGNAL["file_costs"].get(task_id, 0.0) + response.cost_inr
...
def validate_written_files(written: list[str]) -> tuple[bool, str]:
    ...
    if cs_files:
        ...
        for csproj_dir in csproj_dirs:
            ...
            if not csproj_files:
                msg = (
                    f"No .csproj file found in {csproj_dir}. "
            ...
            result = _run(["dotnet", "build", build_target, "--nologo", "-v", "quiet"], check=False, capture=True)
            if result.returncode != 0:
