...
# ── Post comment ──────────────────────────────────────────────────────────────
resp = requests.post(f"{BASE}/issues/{DASHBOARD_ISSUE}/comments", headers=HEADERS, json={"body": comment}, timeout=30)
...
current = requests.get(f"{BASE}/issues/{DASHBOARD_ISSUE}", headers=HEADERS, timeout=30).json()
current_labels = [label["name"] for label in current.get("labels", [])]
new_labels = [label for label in current_labels if label not in sprint_labels] + [label_to_set]
if "FAILED" in RESULT or "HALTED" in (RESULT or ""):
    new_labels.append("founder-action-needed")

requests.patch(f"{BASE}/issues/{DASHBOARD_ISSUE}", headers=HEADERS, json={"labels": new_labels}, timeout=30)
