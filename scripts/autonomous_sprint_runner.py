        ...
        "subtasks": [
            ...
            SubTaskDef(
                ...
                constitutional_check=(
                    ...
                    "\n"
                    "EVALUATION CONTEXT — COMPLETE PROPERTY LIST (read BRANCH CONTEXT for exact types):\n"
                    "  ctx.ContractId           string   — gRPC proto field contract_id\n"
                    "  ctx.ActionType           string   — gRPC proto field action_type\n"
                    "  ctx.ActionParameters     string   — JSON-encoded, use ctx.GetParameter(key) to parse\n"
                    "  ctx.DecisionSpaceVersion int      — gRPC proto field decision_space_version\n"
                    "  ctx.TenantId             string   — from gRPC metadata 'x-tenant-id'\n"
                    "  ctx.SkillId              string?  — optional, from gRPC proto field skill_id\n"
                    "  ctx.ApprovedBudgetInrPaise  long  — from BudgetContext.approved_monthly_budget_inr_paise\n"
                    "  ctx.CurrentSpendInrPaise    long  — from BudgetContext.current_month_spend_inr_paise\n"
                    "  ctx.ProposedSpendInrPaise   long  — from BudgetContext.proposed_spend_inr_paise\n"
                    "  ctx.BudgetSkillType         string — from BudgetContext.skill_type\n"
                    "  ctx.GetParameter(\"key\")   string? — parse JSON from ActionParameters (e.g. 'tool_name')\n"
                    "\n"
                    "⛔ NEVER call: ActionParameters.TryGetValue() — ActionParameters is a string, not a Dictionary.\n"
                    "   INSTEAD use: ctx.GetParameter(\"tool_name\") to get 'tool_name' from the JSON.\n"
                    "⛔ NEVER access: request.TenantId, request.AgentId, request.ToolName — these proto fields do NOT exist.\n"
                    "\n"
                    "HOW EACH EVALUATOR ACCESSES ITS DATA:\n"
                    "  C-041 (Tool Authorization): ctx.GetParameter(\"tool_name\") for tool name; ctx.TenantId for DB read\n"
                    "  C-043 (Budget Ceiling): ctx.ProposedSpendInrPaise, ctx.ApprovedBudgetInrPaise, ctx.CurrentSpendInrPaise\n"
                    "  C-048 (Non-Exploitation): ctx.ActionType directly; ctx.TenantId for cross-tenant check\n"
                    "  C-049 (Honest Limitation): ctx.TenantId + ctx.SkillId for audit query\n"
                    "  C-062 (AI Security): ctx.GetParameter(\"system_prompt_sha\"), ctx.GetParameter(\"message_content_hash\")\n"
                    "\n"
                    "IN ConstitutionalEngineService.ValidateAction:\n"
                    "  Extract tenantId from gRPC metadata: var tenantId = context.RequestHeaders.GetValue(\"x-tenant-id\") ?? \"\";\n"
                    "  Then: var ctx = EvaluationContext.FromRequest(request, tenantId);\n"
                    ...
                    "\n"
                    "EVALUATOR REGISTRY — COMPLETE API (EvaluatorRegistry.cs is already on the branch):\n"
                    "  _registry.EvaluateAllAsync(ctx, cancellationToken) → Task<IReadOnlyList<EvaluationResult>>\n"
                    "  _registry.Count                                      → int\n"
                    "  ⛔ NEVER call: _registry.GetEvaluators()            — does NOT exist\n"
                    "  ⛔ NEVER call: _registry.GetApplicableEvaluators()  — does NOT exist\n"
                    "  ⛔ NEVER call: _registry.Evaluate()                 — does NOT exist\n"
                    "  CORRECT USAGE in ConstitutionalEngineService.ValidateAction:\n"
                    "    var results = await _registry.EvaluateAllAsync(ctx, cancellationToken);\n"
                    "\n"
                    "ICLAMEVALUATOR INTERFACE (already on branch — do NOT redefine):\n"
                    "  string ClaimId { get; }\n"
                    "  Task<EvaluationResult> EvaluateAsync(EvaluationContext context, CancellationToken cancellationToken = default)\n"
                    "\n"
                    "EVALUATIONRESULT RECORD (already on branch — do NOT redefine):\n"
                    "  EvaluationResult(string ClaimId, EvaluationVerdict Verdict, string Reason)\n"
                    "  Constructor: new EvaluationResult(\"C-041\", EvaluationVerdict.Deny, \"reason\")\n"
                    "\n"
                    "PROTO-GENERATED TYPES — EXACT C# FIELDS (from constitutional_service.proto):\n"
                    "ValidateActionResponse fields (return this from ValidateAction):\n"
                    "  Decision:                    ValidationDecision  — use ValidationDecision.Allow / Deny / Escalate\n"
                    "  ConstitutionalBasis:         string              — semicolon-separated claim refs e.g. \"C-041\"\n"
                    "  Reason:                      string              — human-readable explanation\n"
                    "  BudgetRemainingInrPaise:     long?               — only set for budget-related decisions\n"
                    "  SyntheticApprovalRecordId:   string?             — only set for synthetic approvals\n"
                    "  ⛔ NO ClaimId, NO EvaluationResults, NO AllowedActions fields on ValidateActionResponse\n"
                    "  CORRECT usage: return new ValidateActionResponse { Decision = ValidationDecision.Allow, ConstitutionalBasis = \"C-041\", Reason = \"passed\" };\n"
                    "\n"
                    "RecordEvidenceRequest fields (use if calling RecordEvidence — WC012-03):\n"
                    "  ActionInstanceId:            string\n"
                    "  ContractId:                  string\n"
                    "  ProfessionalId:              string\n"
                    "  ActionType:                  string\n"
                    "  State:                       EvidenceState\n"
                    "  ConstitutionalBasis:         string  — required, never empty\n"
                    "  ⛔ NO IdempotencyKey, NO SessionId, NO RequestId fields on RecordEvidenceRequest\n"
                    "  NOTE: Do NOT call RecordEvidence from WC012-02b — that is WC012-03.\n"
                    "\n"
                    "ValidationDecision enum values: Allow, Deny, Escalate (NOT Authorized/Denied)\n"
                    "EvidenceState enum values: Proposed, AwaitingApproval, Approved, Rejected, Executed, Abandoned"
                ),
