...
docker run --rm --user root --network "$NETWORK" \
  ...
  "$RUNNER_IMAGE" sh -lc "cd web && pnpm install --frozen-lockfile --store-dir=/tmp/pnpm-store && pnpm exec playwright test tests/e2e/wc078-public-acquisition.spec.ts --workers=1 --output=/workspace/$EVIDENCE_DIR/playwright --reporter=json > /workspace/$EVIDENCE_DIR/playwright.json"
