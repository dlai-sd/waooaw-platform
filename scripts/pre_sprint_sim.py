...
_RISKY_PATTERNS: dict[str, list[tuple[str, str, str]]] = {
    "python": [
        ...
        (r"sarvam", "SARVAM_SDK", "Sarvam AI — NO Python SDK exists. Use httpx REST calls ONLY."),
        (
            r"asyncio\.run\s*\(",
            "ASYNCIO_RUN_IN_HANDLER",
            "asyncio.run() inside FastAPI/Temporal handler — event loop already running. Use 'await' directly.",
        ),
        (r"\.result\(\)|\.wait\(\)", "SYNC_BLOCK_IN_ASYNC", "Blocking .result()/.wait() in async context. Use 'await' instead."),
    ],
    "dotnet": [
        (
            r"TryGetValue\s*\(",
            "CS1061_TRY_GET_VALUE",
            "TryGetValue on non-dictionary. EvaluationContext.GetParameter() returns string directly.",
        ),
        (r"\.Protos\.", "WRONG_NAMESPACE_PROTOS", "Namespace 'Protos' does not exist. Use Waooaw.ConstitutionalEngine.Grpc"),
        (r"async void\b", "ASYNC_VOID", "async void is dangerous — use async Task instead. Only allowed for event handlers."),
        (r"\bnew \w+Context\(\)", "DBCONTEXT_NO_DI", "DbContext must be injected via DI, never instantiated with new()."),
    ],
    "terraform": [
        (
            r"azurerm_\w+",
            "TERRAFORM_RESOURCE",
            "Azure resource — verify attribute names against azurerm 4.x docs. Never hardcode subscription/tenant IDs.",
        ),
        (r"\".*password.*\"", "TERRAFORM_HARDCODED_SECRET", "Hardcoded password/secret — use var.* or key vault data source."),
    ],
    "typescript": [
        (
            r"useState|useEffect|useRef|onClick|onChange",
            "NEXTJS_CLIENT_NEEDED",
            "Using React hooks or event handlers — add 'use client'; as FIRST line of file.",
        ),
        (
            r"import.*from.*['\"]\.\.\/\.\.\/",
            "NEXTJS_RELATIVE_IMPORT",
            "Deep relative imports — use @/ alias (tsconfig paths). e.g. '@/components/...'",
        ),
        (
            r"useRouter\(\)|usePathname\(\)|useSearchParams\(\)",
            "NEXTJS_ROUTER_HOOK",
...
def _detect_type_gaps(
    spec_text: str, ptr: dict, stack: str, sprint_output_types: set[str] | None = None
) -> list[tuple[str, str]]:
...
def _detect_package_gaps(spec_text: str, ptr: dict, stack: str) -> list[tuple[str, str]]:
    ...
    if stack == "python":
        ...
        for imp in imports:
            if imp not in ("os", "sys", "re", "json", "typing", "datetime", "pathlib", "dataclasses", "enum", "abc", "asyncio"):
            ...
            tasks.append(
                {
                    ...
                    "model_hint": (
                        re.search(r"model_hint.*?(reasoning|auto|none)", body, re.I)
                        or type("", (), {"group": lambda self, n: "reasoning"})()
                    )
                    .group(1)
                    .lower(),
                }
...
def simulate_wc(wc_path: Path) -> WCSimResult:
    ...
    try:
        ...
        print(
            f"  PTR 2.0 assembled: {len(ptr.get(stack, {}).get('types', {}))} {stack} types "
    ...
    ptr_summary = {s: len(ptr.get(s, {}).get("types", {})) for s in ("dotnet", "python", "terraform", "typescript")}
    ...
    for dep in wc_data["dependencies"]:
        ...
        if dep_wc_num:
            dep_src = list((REPO_ROOT / "src").glob("**/*.cs")) + list((REPO_ROOT / "src").glob("**/*.py"))
    ...
    for task in wc_data["tasks"]:
        ...
        for pkg_name, _reason in pkg_gaps[:3]:
            result.gaps.append(
                TaskGap(
                    task_id=task_id,
        ...
        for _, risk_id, description in sdk_risks:
            severity = "CRITICAL" if risk_id in ("SARVAM_SDK", "ASYNCIO_RUN_IN_LOOP", "TERRAFORM_HARDCODED_SECRET") else "HIGH"
            result.gaps.append(
                TaskGap(
                    task_id=task_id,
                    gap_type="SDK_GAP",
    ...
    print(
        f"  Gaps: {len(result.critical_gaps)} CRITICAL · {len(result.high_gaps)} HIGH · "
