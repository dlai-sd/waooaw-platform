...
def _create_next_sprint_simulations(completed_sprint: str) -> None:
    """
    After a sprint merges, create SIM-PL-002 simulation files for the NEXT sprint.
    C-086 (Pre-Execution Simulation Obligation) requires these before any LLM call.
    ...
    next_sprint = f"WC-{next_num:03d}"
    next_sprint_short = f"WC{next_num:03d}"
    ...
    try:
        ...
        if not task_ids:
            print(f"  WARN: No task IDs found for {next_sprint} in TASK_HANDLERS — skipping sim creation")
            return
    except Exception as _e:
        print(f"  WARN: Could not read TASK_HANDLERS ({_e}) — skipping sim creation")
        return
    ...
    for tid in task_ids:
        ...
        sim_path = sim_dir / f"SIM-PL-002-{task_key}.md"
        if sim_path.exists():
            continue  # already exists — don't overwrite
        sim_path.write_text(
            f"# SIM-PL-002 — {task_key}\n"
            f"**Date:** {today}\n"
            f"**Author:** Platform IT Expert (auto-generated post-sprint, Architecture hat)\n"
            f"**Task:** {task_key} — {next_sprint} sprint task\n"
            f"**Simulation type:** Dependency Graph Task Decomposition (IB-021)\n\n"
            f"## Context\n"
            f"Auto-generated by autonomous_sprint_reviewer.py after {completed_sprint} merge.\n"
            f"Validates that {task_key} is in the correct decomposition order and\n"
...
def main() -> int:
    ...
    if approved:
        ...
        if merge_result.returncode == 0:
            ...
            # C-086 requires simulation PASS records before any LLM call in the next sprint.
            # Creating them here (post-merge, before next cron) ensures the gate passes
            # autonomously — no manual intervention needed.
            try:
                _create_next_sprint_simulations(sprint)
            except Exception as _sim_e:
                print(f"  WARN: Next sprint simulation creation failed ({_sim_e}) — non-blocking")
