...
def check_ruff_installed() -> bool:
    r = subprocess.run(["python3", "-m", "ruff", "--version"], capture_output=True, text=True, cwd=REPO_ROOT)  # noqa: S607
    v = r.stdout.strip() or r.stderr.strip()
    return check(
        "ruff installed + functional", r.returncode == 0, v if r.returncode == 0 else "NOT FOUND — add ruff to pip install step"
    )
...
def check_ruff_captures_stderr() -> bool:
    ...
    return check("run_compile_gate captures stdout+stderr", ok, "OK" if ok else "MISSING stderr — silent errors reach no one")
...
def check_ruff_unsafe_fixes() -> bool:
    ...
    return check(
        "ruff --unsafe-fixes in compile gate (F841 auto-rename)",
        ok,
...
def check_tasks_done_parsing() -> bool:
    ...
    if done_block:
        ...
        ok, done = False, []
    return check(
        "parse_sprint_state reads tasks_done as YAML list",
        ok,
...
def check_cumulative_tasks_done() -> bool:
    ...
    ok = "set(tasks_done) | set(tasks_done_state)" in src or "set(tasks_done_state)" in src
    return check(
        "cumulative tasks_done union (never overwrite prior sessions)",
        ok,
...
def check_yaml_syntax() -> bool:
    try:
        ...
        subprocess.run(["pip", "install", "pyyaml", "-q"])  # noqa: S607
        import yaml
...
def check_hyphenated_import_pattern() -> bool:
    ...
    return check(
        "STACK_BEHAVIORAL_RULES has hyphenated-dir import pattern",
        ok,
...
def check_ruff_behavioral_rules() -> bool:
    ...
    ok = len(missing) == 0
    return check(
        "STACK_BEHAVIORAL_RULES has ruff-violation rules (F841/B018/LOG015/G004)",
        ok,
...
def check_cross_session_seed() -> bool:
    ...
    return check("cross-session subtask seed works end-to-end", ok, f"seeded={seeded}" if ok else f"FAILED — seeded={seeded}")
...
def check_merge_syntax() -> bool:
    ...
    code_lines = [line for line in runner_src.splitlines() if not line.strip().startswith("#")]
    code_only = "\n".join(code_lines)
    bad = bool(re.search(r"merge.*--continue.*--no-edit", code_only))
    ok = not bad
    return check(
        "merge conflict resolver uses 'git commit --no-edit' not 'merge --continue'",
        ok,
...
def check_signal_committed() -> bool:
    ...
    return check(
        "signal uses 'git add -f' (file is in .gitignore)",
        ok,
...
def check_signal_before_early_returns() -> bool:
    ...
    return check(
        "signal written before early returns in main() (step 8.1)",
        ok,
        "step 8.1 before PR section" if ok else "MISSING step 8.1 — 'return 0' when no tasks done exits before signal write",
    )
...
def check_empty_signal_guard() -> bool:
    ...
    ok = (
        (".strip()" in cs_src and "is empty" in cs_src)
        or bool(re.search(r"strip\(\).*\nis empty|empty.*strip\(\)", cs_src))
        or "raw_signal" in cs_src
    )
    return check(
        "complete_sprint guards empty signal (no JSONDecodeError crash)",
        ok,
        "guard present" if ok else "MISSING — git show writes empty file when signal not pushed; json.loads('') crashes",
    )
...
def check_complete_sprint_union() -> bool:
    cs_src = (REPO_ROOT / "scripts" / "complete_sprint.py").read_text()
    ok = "set(signal_done) | set(state_done)" in cs_src or ("state_done" in cs_src and "signal_done" in cs_src)
    return check(
        "complete_sprint merges signal+state tasks_done (no overwrite)",
        ok,
        "union merge present" if ok else "MISSING union — PARTIAL run with 0 tasks would erase all prior tasks_done on main",
    )
...
def check_workflow_fetches_sprint_branch() -> bool:
    ...
    ok = (
        'git fetch origin "$SPRINT_BRANCH"' in wf
        or "fetch origin.*SPRINT_BRANCH" in wf
        or bool(re.search(r"git fetch origin.*SPRINT_BRANCH", wf))
    )
    return check(
        "workflow fetches sprint branch before 'git show' signal",
        ok,
        "fetch present" if ok else "MISSING fetch — git show origin/BRANCH:... returns empty if branch not fetched",
    )
...
def check_per_file_ignores() -> bool:
    ...
    ok = len(missing) == 0
    return check(
        "pyproject.toml per-file-ignores covers LOG015, G004 for tests",
        ok,
        "all present" if ok else f"MISSING: {', '.join(missing)} — LLM-generated tests will fail ruff compile gate",
    )
...
"""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        r = subprocess.run(  # noqa: S603
            ["python3", "-m", "ruff", "check", str(src_file), "--config", str(REPO_ROOT / "pyproject.toml")],  # noqa: S607
            capture_output=True,
            text=True,
        )
        if r.returncode != 0:
        ...
        try:
            ...
            subprocess.run(  # noqa: S603
                ["python3", "-m", "ruff", "check", str(test_file), "--fix", "--unsafe-fixes", "--exit-zero"],  # noqa: S607
                capture_output=True,
                text=True,
                cwd=REPO_ROOT,
            )
            r2 = subprocess.run(["python3", "-m", "ruff", "check", str(test_file)], capture_output=True, text=True, cwd=REPO_ROOT)  # noqa: S603, S607
            if r2.returncode != 0:
    ...
    return check(
        "ruff dry-run: src/ + tests/ LLM patterns pass (ANN401/E501/B018/F841/UP042/LOG015/G004)",
        ok,
...
def main() -> int:
    ...
    print("  17 checks · covers all WC-014 + WC-015 failure registry entries")
    print(f"{'=' * 60}\n")
    ...
    if failed == 0:
        ...
        print("     Each failure = 1-4 wasted runs (~30 min each)")
