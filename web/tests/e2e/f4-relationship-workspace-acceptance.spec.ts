...
test('WC095 responsive lifecycle keeps review blockers, actions, and Stop reachable at 200 percent text', async ({ page }, testInfo) => {
  test.skip(!['chromium-expanded', 'chromium-compact-360'].includes(testInfo.project.name), 'Required WC095 viewports only.');
  await page.goto('/relationships/relationship-active');
...
test('WC095 relationship switching and refresh reconstruct only selected relationship review state', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'chromium-expanded', 'One browser proves server reconstruction and relationship isolation.');
  await page.goto('/relationships/relationship-active');
...
test('WC095 payable checkout fails closed without Razorpay configuration and collects no credentials', async ({ page }, testInfo) => {
  test.skip(!['chromium-expanded', 'chromium-compact-360'].includes(testInfo.project.name), 'Required WC095 viewports only.');
  await page.goto('/relationships/relationship-contract');
  await page.getByRole('button', { name: 'Hire and accept exact contract' }).click();
  await page.getByRole('button', { name: 'Continue to payment' }).click();
  await expect(page.locator('.decision-status')).toContainText('Razorpay configuration is pending. No payment was started.');
