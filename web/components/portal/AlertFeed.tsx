...
export function AlertFeed({ initialAlerts, locale }: { initialAlerts: PortalAlert[]; locale: string }) {
  ...
  async function mutate(alert: PortalAlert, action: 'read' | 'acknowledge') {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(typeof result.title === 'string' ? result.title : 'Alert update could not be completed.');
      setAlerts((current) =>
        current.map((item) =>
            ...
            <div className="alert-summary">
              ...
              <div>
                <div className="portal-item-heading">
                  <h2>{alert.source.replaceAll('_', ' ')}</h2>
                  <span className="status-label">{alert.alertType.toLowerCase()}</span>
                  <span className="status-label">{alert.readState.toLowerCase()}</span>
                </div>
                <p>{alert.dueMeaning ?? 'No deadline or due meaning was supplied.'}</p>
                <small>
                  {new Date(alert.occurredAt).toLocaleString(locale)} · {alert.severity.toLowerCase()} severity
                </small>
              </div>
            </div>
            <div className="alert-actions">
              {alert.readState === 'UNREAD' ? (
                <button
                  ...
                  disabled={pendingId === alert.alertId}
              ...
              {alert.availableAction === 'ACKNOWLEDGE' && alert.readState !== 'ACKNOWLEDGED' ? (
                <button
                  className="secondary-command"
                  disabled={pendingId === alert.alertId}
                  onClick={() => void mutate(alert, 'acknowledge')}
                  type="button"
                >
              ...
              {alert.availableAction === 'OPEN' ? (
                <Link className="secondary-link" href={alert.href}>
                  Open <ArrowRight aria-hidden="true" size={18} />
                </Link>
              ) : null}
            </div>
          </li>
