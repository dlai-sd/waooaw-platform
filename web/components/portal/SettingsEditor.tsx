...
export function SettingsEditor({
  ...
  theme,
  timestampVisibility,
}: { locale: string; notificationPreferences: NotificationPreferences; theme: string; timestampVisibility: string }) {
  const [status, setStatus] = useState<'idle' | 'saving' | 'saved' | 'error'>('idle');
  const selectedTheme = theme === 'LIGHT' ? 'LIGHT' : 'DARK';
  async function submit(formData: FormData) {
    setStatus('saving');
    const response = await fetch('/api/identity/settings', {
      method: 'PUT',
  ...
  return (
    ...
    >
      <label>
        Locale
        <input name="locale" defaultValue={locale} required />
      </label>
      <label>
        Theme
      ...
      <label>
        Timestamps
        <select name="timestampVisibility" defaultValue={timestampVisibility}>
      ...
      <output>
        {status === 'saved' ? 'Settings saved.' : status === 'error' ? 'Settings could not be saved.' : ''}
      </output>
    </form>
  );
}
