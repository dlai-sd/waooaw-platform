  ...
  return (
    <>
      <button
        type="button"
        onClick={() => {
          document.title = journey?.current.origin ?? '';
        }}
      >
        Inspect
      </button>
      <output>{journey?.launching ? 'Launching authentication' : 'Idle'}</output>
    </>
  );
}
...
describe('auth public origin', () => {
  ...
  it('owns an ordinary public auth click before the route resolves', () => {
    render(
      <AuthJourney>
        <Link href="/login?returnTo=%2Fprofessionals">Log in</Link>
        <Capture />
      </AuthJourney>
    );
  ...
  it('retains the original public route across login and register switches', () => {
    ...
    render(
      <AuthJourney>
        <Link href="/login" onClick={(event) => event.preventDefault()}>
          Log in
        </Link>
        <Capture />
      </AuthJourney>
  ...
  it('does not capture modified or external navigation', () => {
    ...
    render(
      <AuthJourney>
        ...
        <a href="https://example.com/login" onClick={(event) => event.preventDefault()}>
          External
        </a>
        <Capture />
      </AuthJourney>
