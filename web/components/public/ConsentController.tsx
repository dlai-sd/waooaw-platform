...
export function ConsentController() {
  ...
  function persist(nextAnalytics: boolean, nextAdvertising: boolean) {
    const next = createConsentPreference(
      privacySignal ? false : nextAnalytics,
      privacySignal ? false : nextAdvertising
    );
    ...
    if (!next.analytics || !next.advertising) {
      for (const name of optionalCookieNames) document.cookie = `${name}=; Path=/; Max-Age=0; SameSite=Lax`;
    }
    setReopened(false);
    setSaved(true);
    const consent = optionalConsent(next, privacySignal);
    recordAcquisitionEvent('consent_updated', consent, {
  ...
  return (
    <aside className="consent-banner" aria-label="Cookie preferences">
      <div>
        <strong>{reopened ? 'Review cookie preferences' : 'Your privacy choices'}</strong>
        <p>
          {reopened
            ? 'Update your optional categories. Necessary preferences remain on.'
            : 'Necessary preferences are always on. Optional categories remain off unless selected.'}
          {privacySignal ? ' Your browser privacy signal keeps optional categories off.' : ''}
        </p>
        <label>
          <input
            checked={analytics}
      ...
      <div>
        <button type="button" onClick={() => persist(false, false)}>
          Reject optional
        </button>
        <button type="button" onClick={() => persist(analytics, advertising)}>
          Save preferences
        </button>
        <button className="primary-link" type="button" disabled={privacySignal} onClick={() => persist(true, true)}>
          Accept optional
        </button>
      </div>
    </aside>
