  ...
  return (
    <footer className="public-footer">
      <div className="footer-brand">
        <strong>WAOOAW</strong>
        <p>Governed digital professionals. Visible scope. Reviewable work. Control remains yours.</p>
      </div>
      {siteConfig.footerGroups.map((group) => (
        <nav aria-label={group.label} key={group.label}>
          ...
          {group.label === 'Legal' ? <CookiePreferencesTrigger /> : null}
        </nav>
      ))}
      <div className="footer-contact">
        <strong>Support</strong>
        <a href={`mailto:${siteConfig.contactEmail}`}>{siteConfig.contactEmail}</a>
        <small>{siteConfig.company}</small>
      </div>
      <div className="footer-bottom">
        <span>
          {'\u00a9'} {legal.copyrightYear} {siteConfig.company} {'\u00b7'} CIN: {legal.cin} {'\u00b7'}{' '}
          {legal.registeredLocality}
        </span>
