...
jest.mock('./AppShell', () => ({
  AppShell: ({ applicationControls, children }: { applicationControls: React.ReactNode; children: React.ReactNode }) => <><div>{applicationControls}</div><main>{children}</main></>,
}));

describe('ProtectedAppShell account menu', () => {
  it('closes on an outside pointer interaction', () => {
    render(<ProtectedAppShell locale="en" messages={getMessages('en')} variant="customer"><button type="button">Page action</button></ProtectedAppShell>);
    const toggle = screen.getByLabelText('Account');
    const drawer = toggle.closest('details')!;
  ...
  it('closes on Escape and restores focus to the account control', () => {
    render(<ProtectedAppShell locale="en" messages={getMessages('en')} variant="customer"><span>Content</span></ProtectedAppShell>);
    const toggle = screen.getByLabelText('Account');
    const drawer = toggle.closest('details')!;
