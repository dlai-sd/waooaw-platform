...
export function OnboardForm({ relationshipId, summary }: { relationshipId: string; summary?: string }) {
  ...
  return <form className="portal-form onboard-form" onSubmit={(event) => { event.preventDefault(); void submit(new FormData(event.currentTarget)); }}><p>{summary ?? 'Choose lightweight presentation preferences. Induction continues in conversation.'}</p><label>Agent display name<input name="preferredAgentDisplayName" maxLength={80} placeholder="Optional" /></label><label>Chat appearance<select name="chatAppearance" defaultValue="CONSTITUTIONAL"><option value="CONSTITUTIONAL">Constitutional</option><option value="COMPACT">Compact</option></select></label><label>Timestamps<select name="timestampVisibility" defaultValue="RELATIVE"><option value="RELATIVE">Relative</option><option value="ABSOLUTE">Absolute</option></select></label><label>Theme<select name="themePreference" defaultValue="DARK"><option value="LIGHT">Light</option><option value="DARK">Dark</option></select></label><button className="primary-command" disabled={status === 'saving'} type="submit">{status === 'saving' ? 'Saving…' : 'Save Onboard preferences'}</button><span role="status">{status === 'saved' ? 'Onboard preferences saved.' : status === 'error' ? 'Onboard preferences could not be saved.' : ''}</span></form>;
}