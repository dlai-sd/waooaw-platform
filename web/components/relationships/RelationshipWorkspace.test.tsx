...
const provenance = {
  owner: 'BP',
  sourceProjectionVersion: 'relationship-1',
  producedAt: new Date('2026-08-10T10:00:00Z'),
};
const section = { currencyState: 'UNAVAILABLE' as const, provenance, availableCommands: [] };
const lifecycleStages = (['ONBOARD', 'INDUCT', 'GOAL_VERIFICATION', 'BUSINESS_OUTCOMES', 'OPERATIONS'] as const).map(
  (stage): AgentEmploymentLifecycleStageV1 => ({
    stage,
...
const views: RelationshipWorkspaceViews = {
  workspace: {
    ...
    workspaceVersion: 'relationship-1',
    snapshotState: 'PARTIAL',
    currencyState: 'CURRENT',
    authoritativeCursor: 'workspace:relationship:00000001',
    producedAt: new Date('2026-08-10T10:00:00Z'),
    context: {
      relationshipId: relationship.relationshipId,
  ...
  evidence: {
    schemaVersion: '1.0',
    relationshipId: relationship.relationshipId,
    items: [{ evidenceId: timeline[0].evidenceId, subject: 'TRIAL_STARTED', state: 'RECORDED' }],
  },
};
const evaluation: RelationshipEvaluationProjection = {
  ...
  decisionSpace: {
    version: 1,
    budgetCeilingInrPaise: 100000,
    authorityBoundaries: ['No publishing'],
    stopConditions: ['Customer stop'],
    reviewCadenceMonths: 2,
  },
};
const contractJourney: ContractJourneyProjection = {
  ...
  version: 2,
  contractHash: 'exact-contract-hash',
  relationshipState: 'TRIAL_ACTIVE',
  acceptanceState: 'PENDING',
  paymentState: 'NOT_STARTED',
  activationState: 'NOT_STARTED',
  document: {
    professionalDisplayName: 'Digital Marketing Professional',
    rights: ['Inspect evidence', 'Choose not now'],
...
const performanceReview: PerformanceReviewWindowV1 = {
  ...
  customerBusinessOutcome: {
    state: 'POOR',
    summary: 'Outcome did not improve.',
    evidenceState: 'RECORDED',
    attributionLimits: 'No causal guarantee',
  },
  customerAssessment: {
    state: 'CUSTOMER_DISPUTED',
    summary: 'Customer requested correction.',
    evidenceState: 'RECORDED',
  },
...
describe('RelationshipWorkspace', () => {
  ...
  it('submits an exact-version customer review decision for only the current relationship', async () => {
    ...
    const reviewCall = (global.fetch as jest.Mock).mock.calls.find(([url]) =>
      String(url).endsWith('/performance-reviews')
    );
    ...
    expect(JSON.parse(reviewCall?.[1].body)).toMatchObject({
      command: {
        ...
        payload: {
          ...
          reason: 'Outcome requires a revised plan.',
        },
      },
    });
  });
  ...
  it('presents evaluation state and evidence history', async () => {
    render(
      <RelationshipWorkspace relationship={relationship} timeline={timeline} views={views} evaluation={evaluation} />
    );
    ...
    expect(within(contextNavigation).getByRole('link', { name: 'Usage & budget' })).toHaveAttribute(
      'href',
      '#usage-and-budget'
    );
    expect(within(contextNavigation).getByRole('link', { name: 'Rights & control' })).toHaveAttribute(
      'href',
      '#rights-and-control'
    );
    ...
    const versionContainer = screen.getByText('Version').parentElement;
    if (!versionContainer) throw new Error('Version container is required');
    expect(within(versionContainer).getByText('1')).toBeVisible();
  ...
  it('switches experts by relationship without mutating either relationship', () => {
        ...
        relationships={[
          {
            ...
            agentInstanceId: relationship.agentInstanceId,
            professionalType: relationship.professionalType,
            professionalDisplayName: 'Digital marketing expert',
            lifecycleState: 'TRIAL_ACTIVE',
            unreadState: 'NONE',
          ...
          },
          {
            ...
            agentInstanceId: 'a1a81eb7-6258-45d4-8cc1-c947e029042c',
            professionalType: 'DIGITAL_MARKETING',
            professionalDisplayName: 'Campaign expert',
    ...
    expect(within(switcher).getByRole('link', { name: 'Digital marketing expert' })).toHaveAttribute(
      'aria-current',
      'page'
    );
    expect(within(switcher).getByRole('link', { name: 'Campaign expert' })).toHaveAttribute(
      'href',
      `/relationships/${otherRelationshipId}`
    );
  });
  ...
  it('submits an exact-version skill decision for only the current relationship', async () => {
    ...
    expect(JSON.parse(skillCall?.[1].body)).toMatchObject({
      command: {
        expectedWorkspaceVersion: 'relationship-1',
        expectedSubjectVersion: 'skill-4',
        payload: {
  ...
  it('submits exact-version customer goal verification for only the current relationship', async () => {
    ...
    const goalCall = (global.fetch as jest.Mock).mock.calls.find(([url]) =>
      String(url).endsWith('/goal-verifications')
    );
  ...
  it('keeps failed payment explicitly unresolved', async () => {
    ...
    render(
      <RelationshipWorkspace
        ...
        views={views}
        evaluation={evaluation}
        contractJourney={{ ...contractJourney, acceptanceState: 'ACCEPTED' }}
      />
    );
  ...
  it('renders a truthful non-collecting Demo zero-price checkout', async () => {
    ...
    const activationCall = (global.fetch as jest.Mock).mock.calls.find(
      ([, options]) => typeof options?.body === 'string' && JSON.parse(options.body).action === 'activate'
    );
