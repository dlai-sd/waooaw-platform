...
export function routeContext(pathname: string): {
  surface: PortalInteractionSurfaceV1;
  label: string;
  action: string;
  ...
  if (relationship)
    return {
      ...
      relationshipId: decodeURIComponent(relationship[1]),
    };
  if (pathname.startsWith('/marketplace'))
    return { surface: 'MARKETPLACE', label: 'Marketplace', action: 'Ask about professionals' };
  if (pathname.startsWith('/professionals/mine'))
    return { surface: 'MY_AGENTS', label: 'My Agents', action: 'Discuss my agents' };
  ...
  if (pathname.startsWith('/profile'))
    return {
      surface: pathname.includes('billing') ? 'BILLING' : 'PROFILE',
      label: 'Account',
      action: 'Ask about my account',
    };
...
export function PersistentConversationDock({ locale = 'en-IN' }: { locale?: string }) {
  ...
  return (
    <>
      ...
      <aside
        aria-label={context.relationshipId ? 'Professional conversation' : 'WAOOAW Guide'}
        className="persistent-conversation"
        data-open={open}
        id="persistent-conversation"
        ref={dock}
        style={{ '--guide-width': `${width}px` } as CSSProperties}
      >
        <div
        ...
        <button
          ...
          className="icon-command conversation-close"
          onClick={closeDock}
          ref={closeButton}
          type="button"
        >
        ...
        {context.relationshipId ? (
          <ConversationExperience key={context.relationshipId} relationshipId={context.relationshipId} />
        ) : (
          <PortalGuideExperience currentSurface={context.surface} locale={locale} />
        )}
      </aside>
      {open ? (
        <button
...
export function ConversationContextAction() {
  ...
  return (
    <button
      className="context-conversation-action"
      onClick={(event) =>
