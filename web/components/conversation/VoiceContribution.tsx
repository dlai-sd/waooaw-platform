...
import { CircleAlert, CircleCheck, Mic, Pause, Play, RotateCcw, Send, Square, Trash2 } from 'lucide-react';
...
async function problemTitle(response: Response) {
  ...
  return typeof problem?.title === 'string'
    ? problem.title
    : 'Voice contribution is unavailable. Your text draft is unchanged.';
}
...
export function VoiceContribution({ relationshipId, relationshipStopped, textFallbackId }: VoiceContributionProps) {
  ...
  async function startRecording() {
    ...
    try {
      ...
      setState('error');
      setError(
        caught instanceof Error ? caught.message : 'Microphone permission is unavailable. Use the text message field.'
      );
      focusTextFallback();
    }
  }
  ...
  async function fetchTranscript() {
    ...
    const response = await fetch(
      `/api/voice/${encodeURIComponent(relationshipId)}?sessionId=${encodeURIComponent(sessionId)}&resource=transcript`,
      { cache: 'no-store' }
    );
    ...
    setState('ready');
    setAnnouncement(
      `Transcript ready with ${result.confidenceBand.toLowerCase()} confidence. Review it before sending.`
    );
  }
  ...
  async function sendVoice() {
    ...
    const response = await fetch(`/api/voice/${encodeURIComponent(relationshipId)}`, {
      ...
      body: JSON.stringify({
        ...
        sessionId,
        idempotencyKey: crypto.randomUUID(),
        acceptedTranscriptVersion: version,
      }),
    });
    if (!response.ok) {
      setState('error');
  ...
  return (
    <section className="voice-contribution" aria-labelledby={`voice-title-${relationshipId}`}>
      <div className="voice-heading">
        <div>
          <p className="section-label">Voice draft</p>
          <h3 id={`voice-title-${relationshipId}`}>Record a message</h3>
        </div>
        <span className={`voice-state voice-state-${state}`}>{state.replaceAll('_', ' ')}</span>
      </div>
      <p className="voice-disclosure">
        Microphone audio is transcribed for review. Nothing is sent until you choose{' '}
        <strong>Send voice contribution</strong>. Maximum 3:00 and 15 MiB.
      </p>
      <label className="voice-consent">
        <input
          checked={consent}
          disabled={activeCapture || state === 'sent'}
          onChange={(event) => setConsent(event.target.checked)}
          type="checkbox"
        />
        I agree to record and transcribe this draft.
      </label>
      ...
      <div className="voice-meter" aria-label={`Recording duration ${formatDuration(elapsed)} of 3 minutes`}>
        <span>{formatDuration(elapsed)}</span>
        <progress max={180} value={elapsed} />
      </div>
      <div className="voice-commands">
        {state === 'idle' || state === 'error' ? (
          <button disabled={!consent || relationshipStopped} onClick={() => void startRecording()} type="button">
            <Mic aria-hidden="true" />
            Record
          </button>
        ) : null}
        {activeCapture ? (
          <button onClick={pauseOrResume} type="button">
            {state === 'paused' ? <Play aria-hidden="true" /> : <Pause aria-hidden="true" />}
            {state === 'paused' ? 'Resume' : 'Pause'}
          </button>
        ) : null}
        {activeCapture ? (
          <button onClick={() => recorder.current?.stop()} type="button">
            <Square aria-hidden="true" />
            Stop
          </button>
        ) : null}
        {state !== 'idle' && state !== 'sent' ? (
          <button className="voice-delete" onClick={() => void cancelDraft()} type="button">
            <Trash2 aria-hidden="true" />
            Cancel draft
          </button>
        ) : null}
        {(state === 'review' || state === 'error') && audio ? (
          <button onClick={() => void upload()} type="button">
            <RotateCcw aria-hidden="true" />
            {state === 'review' ? 'Upload for transcript' : 'Reconcile or retry'}
          </button>
        ) : null}
        <button onClick={focusTextFallback} type="button">
          Use text instead
        </button>
      </div>
      {audioUrl ? (
        // biome-ignore lint/a11y/useMediaCaption: This is the user's unsent local recording; no transcript exists, and a text fallback is adjacent.
        <audio controls preload="metadata" src={audioUrl}>
          Audio playback is unavailable. Use text instead.
        </audio>
      ) : null}
      {state === 'ready' || state === 'sending' || state === 'sent' ? (
        <div className="voice-review">
          <div className="voice-confidence">
            {confidence === 'HIGH' ? <CircleCheck aria-hidden="true" /> : <CircleAlert aria-hidden="true" />}Confidence:{' '}
            {confidence.toLowerCase()}
          </div>
          <label htmlFor={`voice-transcript-${relationshipId}`}>Review transcript</label>
          <textarea
            disabled={state === 'sending' || state === 'sent'}
            id={`voice-transcript-${relationshipId}`}
            onChange={(event) => {
          ...
          {!confirmed && state !== 'sent' ? (
            <button onClick={() => void confirmCorrection()} type="button">
              Confirm correction
            </button>
          ) : null}
          <button
            className="send-command"
            disabled={!confirmed || state === 'sending' || state === 'sent' || relationshipStopped}
            onClick={() => void sendVoice()}
            type="button"
          >
            <Send aria-hidden="true" />
            {state === 'sent' ? 'Recorded' : state === 'sending' ? 'Sending' : 'Send voice contribution'}
          </button>
        </div>
      ) : null}
      {error ? (
