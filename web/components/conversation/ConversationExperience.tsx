...
function mergeMessages(current: ConversationMessageV1[], incoming: ConversationMessageV1[]) {
  ...
  for (const message of incoming) canonical.set(message.messageId, message);
...
async function readProblem(response: Response): Promise<string> {
  ...
  return typeof body?.title === 'string'
    ? body.title
    : 'The conversation outcome is unknown. Reconnect before retrying.';
}

async function consumeEventStream(response: Response, onEvent: (event: ConversationStreamEventV1) => Promise<void>) {
...
function Card({
  ...
  return (
    <article
      className={`conversation-card conversation-card-${card.cardType.toLowerCase()}`}
      aria-label={`${card.cardType.toLowerCase()} card`}
    >
              ...
              return (
                <button
                  ...
                  title={
                    enabled
                      ? undefined
                      : (command.unavailableReason ?? 'This command is not available in the current release.')
                  }
                  type="button"
                >
...
function MessageStatus({ message }: { message: ConversationMessageV1 }) {
  ...
  const processing =
    message.processingState === 'RUNNING' || message.processingState === 'QUEUED'
  ...
  const evidence =
    message.evidenceState === 'RECORDED' && Boolean(message.evidenceRecordId)
      ...
      : message.evidenceState === 'RECORDED'
        ...
        : message.evidenceState === 'PENDING'
          ...
          : {
              icon: CircleAlert,
              label: `Evidence ${message.evidenceState.toLowerCase().replaceAll('_', ' ')}`,
              className: '',
            };

  return (
    <div className="message-status" aria-label="Message status">
      {[{ ...delivery, className: '' }, { ...processing, className: '' }, evidence].map(
...
export function ConversationExperience({
    ...
    fetch(`/api/relationships/${encodeURIComponent(relationshipId)}/skill-decisions`, { cache: 'no-store' })
      ...
      .then((payload) => {
        ...
        setSelectedSkillId((current) =>
          payload.skills?.some((skill) => skill.skillId === current) ? current : (payload.skills?.[0]?.skillId ?? '')
        );
      })
      .catch(
        (caught: unknown) =>
          active && setError(caught instanceof Error ? caught.message : 'Accepted Skills are unavailable.')
      );
    return () => {
      active = false;
    };
  ...
  const fetchTimeline = useCallback(
    async (options?: { cursor?: string; afterCursor?: string }) => {
      const search = new URLSearchParams({ limit: '40' });
      if (options?.cursor) search.set('cursor', options.cursor);
      if (options?.afterCursor) search.set('afterCursor', options.afterCursor);
      const response = await fetch(`/api/conversations/${encodeURIComponent(relationshipId)}?${search}`, {
        cache: 'no-store',
      });
      if (!response.ok) throw new Error(await readProblem(response));
  ...
  const submit = useCallback(
    async (contribution: QueuedContribution, cursor: string) => {
      if (!navigator.onLine) {
      ...
      try {
        ...
        setMessages((current) =>
          mergeMessages(
            current.filter((item) => item.messageId !== contribution.clientMessageId),
            [submission.message]
          )
        );
        ...
        setMessages((current) =>
          current.map((item) =>
            item.messageId === contribution.clientMessageId ? { ...item, deliveryState: 'UNRESOLVED' } : item
          )
        );
      } finally {
        setSending(false);
  ...
  const reconcileContribution = useCallback(
    async (contribution: QueuedContribution, page: ConversationTimelinePageV1) => {
  ...
  useEffect(() => {
    ...
    fetchTimeline()
      ...
      .catch(
        (caught: unknown) => active && setError(caught instanceof Error ? caught.message : 'Conversation unavailable.')
      )
    ...
    <section className="conversation-experience" aria-labelledby="conversation-title">
      <header className="conversation-heading">
        <div>
          ...
          <h2 id="conversation-title">Work with your professional</h2>
        </div>
      ...
      {nextCursor ? (
        <button className="load-older" onClick={() => void fetchTimeline({ cursor: nextCursor })} type="button">
      ...
      <div className="conversation-timeline" aria-busy={loading} aria-label="Conversation timeline">
        {loading ? (
          <p className="conversation-empty">
            <LoaderCircle aria-hidden="true" className="spin" /> Loading conversation…
          </p>
        ) : null}
        {!loading && messages.length === 0 ? (
          <p className="conversation-empty">No messages yet. Start with a clear outcome for your professional.</p>
        ) : null}
        {messages.map((message) => {
          const date = message.acceptedAt.toLocaleDateString(locale, {
            day: 'numeric',
            month: 'long',
            year: 'numeric',
          });
          ...
          return (
            <div key={message.messageId}>
              {showDate ? (
                <p className="conversation-separator">
                  <span>{date}</span>
                </p>
              ) : null}
              ...
              <article className={`conversation-message message-${message.actor.toLowerCase()}`}>
                ...
                {message.partial ? (
                  <p className="partial-disclosure">
                    Incomplete response · {message.completionReason?.replaceAll('_', ' ') ?? 'still changing'}
                  </p>
                ) : null}
                ...
                {message.deliveryState === 'FAILED' || message.deliveryState === 'UNRESOLVED' ? (
                  <button className="retry-command" onClick={() => void retry(message)} type="button">
      ...
      >
        ...
        >
          {acceptedSkills.length ? null : <option value="">No accepted Skills available</option>}
          {acceptedSkills.map((skill) => (
            <option key={`${skill.skillId}:${skill.skillVersion}`} value={skill.skillId}>
              {skill.skillId.replaceAll('_', ' ').toLowerCase()} · {skill.skillVersion}
            </option>
          ))}
        </select>
        ...
        <div className="composer-commands">
          <span>
            {connection === 'offline' || queued
              ? 'Draft retained on this device until reconciliation.'
              : 'Enter sends only with the Send button.'}
          </span>
          {executionId ? (
            <button className="cancel-command" onClick={() => void cancel()} type="button">
          ...
          <button
            ...
            disabled={!draft.trim() || !selectedSkillId || sending || Boolean(queued) || connection === 'stopped'}
            type="submit"
          >
            {sending ? (
