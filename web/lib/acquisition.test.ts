...
describe('acquisition event boundary', () => {
  ...
  it.each(['email', 'tenant_id', 'message', 'raw_referrer'])('rejects prohibited field %s', (field) =>
    expect(validateAcquisitionEvent({ ...valid, [field]: 'private' })).toEqual({ ok: false, error: 'PROHIBITED_FIELD' })
  );
  it('rejects unsafe attribution', () =>
    expect(validateAcquisitionEvent({ ...valid, utm_source: 'x'.repeat(101) })).toEqual({
      ok: false,
      error: 'INVALID_ATTRIBUTION',
    }));
  it.each(['timestamp', 'locale', 'environment', 'consent', 'deduplication_id'])(
    'rejects a missing common field %s',
    (field) => {
  ...
  it('rejects missing event-specific data', () =>
    expect(validateAcquisitionEvent({ ...valid, event_name: 'contact_invoked' })).toEqual({
      ok: false,
      error: 'MISSING_EVENT_DATA',
    }));
  it('rejects a malformed consent snapshot', () =>
    expect(
      validateAcquisitionEvent({ ...valid, consent: { analytics: true, advertising: false, account_id: 'private' } })
