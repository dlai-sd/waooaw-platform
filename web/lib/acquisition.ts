...
export function validateAcquisitionEvent(input: unknown): AcquisitionValidation {
  ...
  if (
    ...
    !eventId.test(event.event_id) ||
    typeof event.deduplication_id !== 'string' ||
    !eventId.test(event.deduplication_id) ||
    typeof event.timestamp !== 'string' ||
    Number.isNaN(Date.parse(event.timestamp)) ||
    typeof event.route_id !== 'string' ||
    !boundedValue.test(event.route_id) ||
  ...
  for (const key of [...fields.required, ...fields.optional]) {
    if (event[key] !== undefined && (typeof event[key] !== 'string' || !boundedValue.test(event[key] as string))) {
      if (
        ...
        (key !== 'analytics' && key !== 'advertising') ||
        typeof event[key] !== 'boolean'
      )
        return { ok: false, error: 'INVALID_EVENT_DATA' };
    }
  }
  ...
  if (
    event.referrer_class !== undefined &&
    (typeof event.referrer_class !== 'string' || !referrerClasses.has(event.referrer_class))
  )
    return { ok: false, error: 'INVALID_ATTRIBUTION' };
