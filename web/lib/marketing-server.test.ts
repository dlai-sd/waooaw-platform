...
const readyEnvironment = {
  ...
  SERVER_GTM_ENABLED: 'true',
  SERVER_GTM_ENDPOINT: 'https://metrics.example.test/events',
  SERVER_GTM_ALLOWED_HOSTS: 'metrics.example.test',
  SERVER_GTM_AUTH_TOKEN: 'gtm-secret',
  META_ENABLED: 'true',
...
describe('server marketing destinations', () => {
  ...
  it('disables incomplete, unapproved, and non-consented destinations', async () => {
    ...
    await dispatchAcquisitionEvent(
      { ...event, consent: { analytics: true, advertising: false } },
      fetcher,
      environment
    );
  ...
  it('does not route consent updates and contains destination failures', async () => {
    const fetcher = jest.fn<ReturnType<typeof fetch>, Parameters<typeof fetch>>(async () => {
      throw new Error('destination unavailable');
    });
    await expect(
      dispatchAcquisitionEvent({ ...event, event_name: 'consent_updated' }, fetcher, readyEnvironment)
    ).resolves.toEqual([]);
    await expect(dispatchAcquisitionEvent(event, fetcher, readyEnvironment)).resolves.toEqual([
