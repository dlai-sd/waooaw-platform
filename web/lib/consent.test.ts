...
describe('versioned consent preference', () => {
  it('round trips only the current granular preference', () => {
    ...
    expect(parseConsentCookie(`${consentCookieName}=${encodeURIComponent(JSON.stringify(preference))}`)).toEqual(
      preference
    );
  });
  it('rejects stale and malformed preferences', () => {
    ...
    expect(
      parseConsentCookie(
        `${consentCookieName}=${encodeURIComponent(JSON.stringify({ ...createConsentPreference(true, true), policyVersion: 'old' }))}`
      )
    ).toBeNull();
  });
  it('lets DNT or GPC override every optional category', () =>
    expect(optionalConsent(createConsentPreference(true, true), true)).toEqual({
      analytics: false,
      advertising: false,
    }));
});
