...
function approvedGtmEndpoint(environment: MarketingEnvironment): string | undefined {
  ...
  try {
    ...
    const hosts = new Set(
      (environment.SERVER_GTM_ALLOWED_HOSTS ?? '')
        .split(',')
        .map((host) => host.trim())
        .filter(Boolean)
    );
...
export async function dispatchAcquisitionEvent(
  ...
  if (
    ...
    enabled(environment.GA4_ENABLED) &&
    /^G-[A-Z0-9]+$/.test(environment.GA4_MEASUREMENT_ID ?? '') &&
    ...
    deliveries.push(
      deliver('ga4', () =>
        fetcher(endpoint, {
          ...
          body: JSON.stringify({
            client_id: event.deduplication_id,
            events: [{ name: event.event_name, params: event }],
          }),
  ...
  if (
    event.consent.advertising &&
    enabled(environment.META_ENABLED) &&
    /^\d{5,30}$/.test(environment.META_PIXEL_ID ?? '') &&
            ...
            data: [
              {
                ...
                event_time: Math.floor(Date.parse(event.timestamp) / 1000),
