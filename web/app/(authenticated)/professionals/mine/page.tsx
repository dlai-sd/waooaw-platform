...
export default async function MyProfessionalsPage() {
	...
	try {
		...
		return <section className="portal-page" aria-labelledby="my-experts-title"><header className="portal-heading"><p className="eyebrow">Your team</p><h1 id="my-experts-title">{messages.myExperts}</h1><p>Each expert keeps a separate contract, goals, skills, trial and workspace.</p></header><ul className="marketplace-grid">{page.items.map((item) => <li key={item.relationshipId}><div className="portal-item-heading"><h2>{item.professionalDisplayName}</h2><span className="status-label">{(item.trialStatus === 'EXPIRED' ? item.trialStatus : item.lifecycleState).replaceAll('_', ' ')}</span></div><p>{item.professionalType}</p><p>{item.currentGoalSummary ?? 'No current goal has been confirmed.'}</p><footer><span>{item.availabilityState.replaceAll('_', ' ')}</span><Link className="secondary-link" href={`/relationships/${item.relationshipId}`}>Open workspace <ArrowRight aria-hidden="true" size={18} /></Link></footer></li>)}</ul></section>;
	} catch {
