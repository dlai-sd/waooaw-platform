  ...
  if (!session?.authenticated)
    return (
      <section className="auth-view">
        <h1>{messages.duplicate}</h1>
        <SignInCommand callbackUrl="/account-link" label={messages.signInFirst} />
      </section>
    );
  return (
    <StateView
      ...
      description="Continue from the verified WhatsApp conversation. No link will be created until the internal channel proof and fresh account assurance are both available."
    />
  );
}
