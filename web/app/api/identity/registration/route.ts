...
async function safeRegistrationProblem(error: ResponseError) {
  const upstream = (await error.response
    .clone()
    .json()
    .catch(() => undefined)) as { code?: unknown; correlationId?: unknown } | undefined;
  ...
  const correlationId =
    ...
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(upstream.correlationId)
      ...
      : undefined;
  const status =
    approved && [400, 401, 403, 404, 409, 410, 422, 429, 503].includes(error.response.status)
      ? error.response.status
      : 503;
  return {
    status,
    body: { code, title: 'Identity request could not be completed.', ...(correlationId ? { correlationId } : {}) },
  };
}
    ...
    switch (action) {
      case 'start': {
        ...
        try {
          ...
          if (
            init.signal.aborted ||
            typeof session?.accountReference !== 'string' ||
            !session.accountReference.trim() ||
        ...
        return NextResponse.json(
          await api.startIdentityRegistration(
            {
              idempotencyKey,
      ...
      case 'profile':
        return NextResponse.json(
          await api.updateIdentityRegistrationProfile({
      ...
      case 'email-start':
        return NextResponse.json(
          await api.startIdentityEmailVerification({
            registrationId: requiredString(body, 'registrationId'),
            idempotencyKey,
      ...
      case 'email-confirm':
        return NextResponse.json(
          await api.confirmIdentityEmailVerification({
            registrationId: requiredString(body, 'registrationId'),
            idempotencyKey,
      ...
      case 'mobile-start':
        return NextResponse.json(
          await api.startIdentityMobileVerification({
            registrationId: requiredString(body, 'registrationId'),
            idempotencyKey,
      ...
      case 'mobile-confirm':
        return NextResponse.json(
          await api.confirmIdentityMobileVerification({
            registrationId: requiredString(body, 'registrationId'),
            idempotencyKey,
            confirmIdentityVerificationRequest: {
              challengeId: requiredString(body, 'challengeId'),
      ...
      case 'complete': {
        ...
        try {
          const completion = await api.completeIdentityRegistration(
            {
              registrationId: requiredString(body, 'registrationId'),
              idempotencyKey,
            },
          ...
          if (
            !completion ||
            !['ACCOUNT_CREATED', 'ACCOUNT_REUSED'].includes(completion.outcome) ||
            typeof completion.accountReference !== 'string' ||
            !completion.accountReference.trim()
          ) {
            throw new Error();
          }
          const session = await api.getIdentitySession(init);
          if (
            init.signal.aborted ||
            !session ||
            session.accountReference !== completion.accountReference ||
            !(session.expiresAt instanceof Date) ||
            !(session.expiresAt.getTime() > Date.now())
          ) {
            throw new Error();
