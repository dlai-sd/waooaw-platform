...
describe('portal interaction boundary', () => {
  ...
  it('forwards only validated portal context through the authenticated generated client', async () => {
    ...
    const response = await POST(new NextRequest('http://localhost/api/interactions/portal', {
      method: 'POST', body: JSON.stringify({ idempotencyKey: 'key-1', clientMessageId: 'message-1', text: 'Show agents', locale: 'en-IN', currentSurface: 'MY_AGENTS' }),
    }));

    expect(response.status).toBe(200);
    expect(mockCreateApi).toHaveBeenCalledWith('server-token');
    expect(sendPortalInteractionMessage).toHaveBeenCalledWith(expect.objectContaining({
      idempotencyKey: 'key-1', sendPortalInteractionMessageRequestV1: expect.objectContaining({ currentSurface: 'MY_AGENTS' }),
    }));
  });
  ...
  it('fails closed without a session or with a browser-invented surface', async () => {
    ...
    const invalid = await POST(new NextRequest('http://localhost/api/interactions/portal', {
      method: 'POST', body: JSON.stringify({ idempotencyKey: 'key-1', clientMessageId: 'message-1', text: 'Do work', locale: 'en-IN', currentSurface: 'ADMIN' }),
    }));
