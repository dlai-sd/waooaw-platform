...
export async function POST(request: NextRequest) {
  ...
  if (!body.professionalType || !/^[A-Z][A-Z0-9_]{0,63}$/.test(body.professionalType)
    || !body.professionalVersion || !versionPattern.test(body.professionalVersion)
    || (body.intent !== 'trial' && body.intent !== 'hire')
    || !body.disclosureRevision || !versionPattern.test(body.disclosureRevision)
    || !body.termsVersion || !/^\d{4}-\d{2}-\d{2}$/.test(body.termsVersion)
    || !body.idempotencyKey || !uuidPattern.test(body.idempotencyKey)) {
    return NextResponse.json({ title: 'Acquisition continuation is invalid.' }, { status: 400 });
  }

  const api = new ProfessionalsApi(new Configuration({
  ...
  try {
    ...
    return NextResponse.json({ title: 'Acquisition could not continue.' }, { status: 503, headers: { 'Cache-Control': 'no-store' } });
  }
}