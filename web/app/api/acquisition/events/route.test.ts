...
const request = (headers: HeadersInit = {}, body: unknown = event) =>
  new Request('https://waooaw.com/api/acquisition/events', {
    method: 'POST',
    headers: { 'content-type': 'application/json', referer: 'https://waooaw.com/', ...headers },
    body: JSON.stringify(body),
  });

describe('acquisition route consent enforcement', () => {
  beforeEach(() => dispatch.mockClear());
  it('does not trust an arbitrary consent header', async () =>
    expect((await POST(request({ 'x-waooaw-consent': 'analytics=true' }))).status).toBe(403));
