...
export async function GET(request: NextRequest, { params }: { params: Promise<{ relationshipId: string }> }) {
  ...
  const response = await fetch(
    `${businessPlatformUrl}/api/v1/employment/relationships/${encodeURIComponent(relationshipId)}/evaluation`,
    {
  ...
  if (!Array.isArray(payload.skills)) {
    return NextResponse.json(
      { title: 'Accepted Skills are unavailable.' },
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ relationshipId: string }> }) {
  ...
  if (!body.idempotencyKey || !body.command)
    return NextResponse.json({ title: 'Skill decision is invalid.' }, { status: 400 });
  const response = await fetch(
