...
const createIdentityApi = jest.fn<
  Pick<
    IdentityApi,
...
jest.mock('@/lib/api/generated/apis/NotificationsApi', () => ({
  NotificationsApi: jest.fn(() => ({ markCustomerAlertRead, acknowledgeCustomerAlert })),
}));
jest.mock('@/lib/api/generated/apis/ConfigurationApi', () => ({
  ConfigurationApi: jest.fn(() => ({ updateRelationshipOnboard })),
}));
...
describe('WC085 registration handoff', () => {
  ...
  const completion = {
    ...
    defaultTarget: 'APPLICATION_HOME',
  };
  ...
  async function complete() {
    ...
    return POST(
      new NextRequest('http://localhost/api/identity/registration', {
        method: 'POST',
        body: JSON.stringify({ action: 'complete', registrationId: 'registration-1', idempotencyKey: 'key-1' }),
      })
    );
  }
  ...
  it.each(['ACCOUNT_CREATED', 'ACCOUNT_REUSED'])(
    'confirms %s with the same server token and no account or session payload',
    async (outcome) => {
      ...
      expect(completeIdentityRegistration).toHaveBeenCalledWith(
        { registrationId: 'registration-1', idempotencyKey: 'key-1' },
        expect.objectContaining({ cache: 'no-store', signal: expect.any(AbortSignal) })
      );
      expect(getIdentitySession).toHaveBeenCalledWith(
        expect.objectContaining({ cache: 'no-store', signal: expect.any(AbortSignal) })
      );
      expect(completeIdentityRegistration.mock.invocationCallOrder[0]).toBeLessThan(
        getIdentitySession.mock.invocationCallOrder[0]
      );
    }
  );
  ...
  it.each([
    ...
    null,
    {},
    { ...completion, accountReference: '' },
    { ...completion, accountReference: ' ' },
    { ...completion, outcome: 'PENDING' },
  ])('rejects an incomplete completion result: %p', async (result) => {
  ...
  it.each([
    ...
    null,
    {},
    { ...session, accountReference: '' },
    { ...session, accountReference: 'other-account' },
    { ...session, expiresAt: new Date(0) },
    { ...session, expiresAt: new Date('invalid') },
  ])('rejects an unconfirmed session without leaking completion: %p', async (result) => {
  ...
  it.each(['completion', 'session'])('sanitizes raw %s failures', async (stage) => {
    ...
    const error = new ResponseError(
      new Response(JSON.stringify({ title: 'private-token private-tenant other-account', accountReference }), {
        status: 403,
      })
    );
  ...
  it.each(['matching', 'mismatched', 'unavailable'])(
    'uses the generated client with the BP wire shape and a %s session',
    async (state) => {
      ...
      const wireSession = {
        ...
        nextAction: 'CONTINUE_TO_DEFAULT_TARGET',
      };
      const fetchMock = jest
        ...
        .mockResolvedValueOnce(
          new Response(
            JSON.stringify(state === 'unavailable' ? { title: 'private-token other-account' } : wireSession),
  ...
  it('leaves the generated direct GET registration operation unchanged', async () => {
    ...
    try {
      const result = await realIdentity
        .createIdentityApi('server-token')
        .getIdentityRegistration({ registrationId: 'registration-1' }, { cache: 'no-store' });
  ...
  it('sanitizes an upstream start error without forwarding its raw body', async () => {
    ...
    getIdentitySession.mockRejectedValue(
      new ResponseError(new Response(JSON.stringify({ code: 'REGISTRATION_REQUIRED' }), { status: 409 }))
    );
    startIdentityRegistration.mockRejectedValue(
      new ResponseError(
        new Response(JSON.stringify({ title: 'private-token private-tenant', accountReference }), { status: 409 })
      )
    );
    const { POST } = await import('./identity/registration/route');
    const response = await POST(
      new NextRequest('http://localhost/api/identity/registration', {
        method: 'POST',
        body: JSON.stringify({ action: 'start', languagePreference: 'en', idempotencyKey: 'key-1' }),
      })
    );
    expect(response.status).toBe(503);
    expect(await response.json()).toEqual({
  ...
  it.each([
    ...
    { ...session, accountReference: '' },
    { ...session, expiresAt: new Date(0) },
    new Error('private-session'),
  ])('does not start registration on an unresolved session: %p', async (result) => {
...
describe('WC084 portal server boundaries', () => {
  ...
  it('validates and forwards a trimmed profile command', async () => {
    ...
    expect(updateCustomerProfile).toHaveBeenCalledWith(
      expect.objectContaining({
        idempotencyKey: 'key',
        updateCustomerProfileRequestV1: expect.objectContaining({
          displayName: 'Asha',
          organizationDisplayName: 'Acme',
        }),
      }),
  ...
  it('preserves identity failures for profile and settings', async () => {
    ...
    expect(
      (
        await settings.PUT(
          request('/api/identity/settings', { settings: { schemaVersion: '1.0.0' }, idempotencyKey: 'key' })
        )
      ).status
    ).toBe(503);
  });
  ...
  it('validates and forwards settings with the server token', async () => {
    ...
    expect(response.status).toBe(200);
    expect(updateCustomerSettings).toHaveBeenCalledWith(
      { idempotencyKey: 'key', updateCustomerSettingsRequestV1: settings },
      { cache: 'no-store' }
    );
  });
  ...
  it('validates and forwards alert read and acknowledge commands', async () => {
    ...
    expect(
      (await POST(new NextRequest('http://localhost/api/alerts/alert-1', { method: 'POST', body: '{}' }), params))
        .status
    ).toBe(400);
    ...
    const read = new NextRequest('http://localhost/api/alerts/alert-1', {
      method: 'POST',
      body: JSON.stringify({ action: 'read', expectedAlertVersion: '1', idempotencyKey: 'key' }),
    });
    const acknowledge = new NextRequest('http://localhost/api/alerts/alert-1', {
      method: 'POST',
      body: JSON.stringify({ action: 'acknowledge', expectedAlertVersion: '2', idempotencyKey: 'key-2' }),
    });
    expect((await POST(read, params)).status).toBe(200);
    expect((await POST(acknowledge, params)).status).toBe(200);
    expect(markCustomerAlertRead).toHaveBeenCalledWith(
      expect.objectContaining({ alertId: 'alert-1', idempotencyKey: 'key' }),
      { cache: 'no-store' }
    );
  ...
  it('validates and forwards Onboard preferences', async () => {
    ...
    const response = await PUT(
      request('/api/relationships/relationship-1/onboard', { onboard, idempotencyKey: 'key' }),
      params
    );
    expect(response.status).toBe(200);
    expect(updateRelationshipOnboard).toHaveBeenCalledWith(
      { relationshipId: 'relationship-1', idempotencyKey: 'key', relationshipOnboardRequestV1: onboard },
      { cache: 'no-store' }
    );
  });
  ...
  it('requires authentication and fails closed for unresolved Onboard updates', async () => {
    ...
    expect(
      (
        await PUT(
          request('/api/relationships/relationship-1/onboard', {
            onboard: { schemaVersion: '1.0.0' },
            idempotencyKey: 'key',
          }),
          params
        )
