...
export default async function ProfessionalPage({ params, searchParams }: ProfessionalPageProps) {
	...
	if (hasAcquisitionQuery && (
		!query?.professionalType
		|| !/^[A-Z][A-Z0-9_]{0,63}$/.test(query.professionalType)
		|| !query.version
	...
	const disclosure = hasAcquisitionQuery
		? await getProfessionalDisclosure(query!.professionalType!).catch(() => undefined)
		: undefined;
	if (hasAcquisitionQuery && (
		!disclosure
	...
	return (
		<article className="public-document professional-detail">
			<StructuredData value={[{ '@context': 'https://schema.org', '@type': 'Service', name: displayName, description: publication.summary, url: absoluteUrl(path), provider: { '@type': 'Organization', name: 'WAOOAW' } }, breadcrumbData(displayName, path)]} />
			<header><p className="eyebrow">{publication.domain}</p><h1>{displayName}</h1><p>{publication.summary}</p></header>
			<section><h2>What this professional can help with</h2><ul>{suitability.map((item) => <li key={item}>{item}</li>)}</ul></section>
			{disclosure ? <section><h2>Skills included</h2><ul>{disclosure.skills.map((skill) => <li key={skill.skillId}>{skill.displayName}{skill.applicableInTrial ? ' (available in trial)' : ''}</li>)}</ul></section> : null}
			<section><h2>Honest limits</h2><ul>{limitations.map((item) => <li key={item}>{item}</li>)}</ul></section>
			{disclosure ? <>
				<section><h2>Your control</h2><ul>{disclosure.customerRights.map((item) => <li key={item}>{item}</li>)}</ul></section>
				<section><h2>Before live work</h2><ul>{disclosure.authorityNeeds.map((item) => <li key={item}>{item}</li>)}</ul></section>
				<p className="price-disclosure"><strong>{new Intl.NumberFormat('en-IN', { style: 'currency', currency: disclosure.indicativePrice.currency }).format(disclosure.indicativePrice.amountInrPaise / 100)}</strong> / {disclosure.indicativePrice.cadence.toLowerCase()}<small>{disclosure.indicativePrice.qualification}</small></p>
			</> : null}
			<p className="publication-note">Disclosure {disclosureRevision} for professional release {version}. Availability is confirmed before continuation.</p>
			<DisclosureContinuation disclosureRevision={disclosureRevision} initialIntent={intent} professionalType={professionalType} professionalVersion={version} termsVersion={termsVersion} trialAvailable={disclosure?.trial.available ?? true} />
		</article>
	);
}