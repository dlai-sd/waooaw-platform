...
export function generateStaticParams() {
  return listPublishedArticles().map(({ slug }) => ({ slug }));
}
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const article = getPublishedArticle((await params).slug);
  if (!article) notFound();
  return publicMetadata(`${article.title} | WAOOAW`, article.description, `/blogs/${article.slug}`, 'article');
}
export default async function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="public-document blog-detail">
      <StructuredData
        value={[
          {
          ...
          breadcrumbData(article.title, path),
        ]}
      />
      <header>
      ...
      <a className="primary-link" href={`/professionals/${article.relatedProfessional}`}>
        Explore the related professional
      </a>
    </article>
  );
}
