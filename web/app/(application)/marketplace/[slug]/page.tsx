...
export default async function MarketplaceOfferPage({
  ...
  return (
    <section className="portal-page marketplace-detail" aria-labelledby="offer-title">
      <Link className="offer-back" href="/marketplace">
      ...
      <header className="offer-detail-heading">
        <div>
          ...
          <h1 id="offer-title">{disclosure.displayName}</h1>
          <p>{disclosure.suitability[0]}</p>
        </div>
        <span className="offer-mark offer-mark-large">
      ...
      <div className="offer-detail-grid">
        <section className="offer-detail-main" aria-labelledby="included-title">
          <p className="offer-intent">
            <BadgeCheck aria-hidden="true" size={18} />
            You chose to {intent === 'trial' ? 'start a trial' : 'hire this professional'}.
          </p>
          <h2 id="included-title">What you can achieve</h2>
          <ul className="offer-outcomes offer-outcomes-detail">
            {disclosure.suitability.map((item, index) => (
              <li key={item}>
                {index === 0 ? <Target aria-hidden="true" size={20} /> : <BarChart3 aria-hidden="true" size={20} />}
                <span>{item}</span>
              </li>
            ))}
          </ul>
          <h2>Included capabilities</h2>
          <ul className="offer-capabilities">
            {disclosure.skills.map((skill) => (
              <li key={skill.skillId}>
                <BadgeCheck aria-hidden="true" size={18} />
                <span>
                  <strong>{skill.displayName}</strong>
                  {skill.applicableInTrial ? <small>Included in trial</small> : null}
                </span>
              </li>
            ))}
          </ul>
          <details className="offer-details">
            ...
            <div>
              <h3>Honest limits</h3>
              <ul>
                {disclosure.limitations.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
              <h3>Your control</h3>
        ...
        <aside className="offer-decision" aria-label={`${intent === 'trial' ? 'Trial' : 'Hire'} summary`}>
          ...
          <p className="eyebrow">
            {intent === 'trial' ? `${disclosure.trial.durationDays}-day trial` : 'Professional plan'}
          </p>
          <p className="offer-decision-price">
            <strong>{intent === 'trial' ? 'No paid tools' : price}</strong>
            {intent === 'hire' ? (
              <span>/ {disclosure.indicativePrice.cadence.toLowerCase()}</span>
            ) : (
              <span>during your trial</span>
            )}
          </p>
          <p>
            {intent === 'trial'
              ? 'Explore planning, research and strategy without publishing, external actions or paid API usage.'
              : 'Final included capability and provider costs are confirmed before contract acceptance.'}
          </p>
