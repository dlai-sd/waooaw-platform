...
function EmptyMyAgentsWorkspace() {
  return (
    <section className="agent-empty-primary" aria-labelledby="my-agents-empty-title">
      ...
      <p>
        Browse available professionals, compare their scope and terms, and begin registration only when you choose Trial
        or Hire.
      </p>
      <Link className="primary-link" href="/marketplace">
  ...
  if (!accessToken)
    return (
      <StateView
        ...
        description="Sign in again to view your employed professionals."
      />
    );
  try {
    ...
    return (
      <section className="portal-page" aria-labelledby="my-experts-title">
          ...
          {page.items.map((item) => (
            <li key={item.relationshipId}>
              ...
              <dl className="agent-facts">
                <div>
                  <dt>Setup</dt>
                  <dd>{item.configurationState.replaceAll('_', ' ')}</dd>
                </div>
                <div>
                  <dt>Skills</dt>
                  <dd>
                    {item.enabledSkillCount} enabled · {item.pendingSkillCount} pending
                  </dd>
                </div>
                <div>
                  <dt>Goal</dt>
                  <dd>{item.currentGoalSummary ?? 'No verified goal yet.'}</dd>
                </div>
                <div>
                  <dt>Work</dt>
                  <dd>{item.currentWorkSummary ?? 'No current work reported.'}</dd>
                </div>
                <div>
                  <dt>Performance</dt>
                  <dd>{item.performanceSummary}</dd>
                </div>
                <div>
                  <dt>Billing</dt>
                  <dd>{item.billingSummary}</dd>
                </div>
              </dl>
              {item.blockerSummary ? <p className="portal-error">{item.blockerSummary}</p> : null}
              <footer>
                <span>
                  {item.availabilityState.replaceAll('_', ' ')} · confirmed{' '}
