...
const valid = () =>
  JSON.parse(JSON.stringify({ site: siteConfig, theme: themeConfig, marketing: marketingConfig, messages })) as {
...
describe('public configuration schema', () => {
  ...
  it.each([
    ...
    [
      'unsafe URL',
      (value: ReturnType<typeof valid>) => {
        value.site.canonicalOrigin = 'javascript:alert(1)';
      },
      'CANONICAL_ORIGIN_UNSAFE',
    ],
    [
      'invalid color',
      (value: ReturnType<typeof valid>) => {
        value.theme.brandBlue = 'blue';
      },
      'THEME_COLOR_INVALID',
    ],
    [
      'missing locale key',
      (value: ReturnType<typeof valid>) => {
        value.messages.ur = Object.fromEntries(Object.entries(value.messages.ur).filter(([key]) => key !== 'register'));
      },
    ...
    [
      'secret-like key',
      (value: ReturnType<typeof valid>) => {
        value.marketing = { ...value.marketing, apiSecret: 'not-public' };
      },
      'PUBLIC_SECRET_INVALID',
    ],
