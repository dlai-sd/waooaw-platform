...
export function validatePublicConfiguration(configuration: PublicConfiguration): string[] {
  ...
  try {
    ...
    if (origin.protocol !== 'https:' && origin.hostname !== 'localhost' && origin.hostname !== '127.0.0.1')
      errors.push('CANONICAL_ORIGIN_UNSAFE');
  } catch {
    errors.push('CANONICAL_ORIGIN_INVALID');
  }
  ...
  if (
    ...
    locales.join('|') !== supportedLocales.join('|') ||
    new Set(locales).size !== locales.length
  )
    errors.push('LOCALES_INVALID');
  const navigation = configuration.site.publicNavigation as { href?: unknown }[];
  if (
    !Array.isArray(navigation) ||
    navigation.some(({ href }) => typeof href !== 'string' || !href.startsWith('/')) ||
    new Set(navigation.map(({ href }) => href)).size !== navigation.length
  )
    errors.push('NAVIGATION_INVALID');
  const colorValues = Object.entries(configuration.theme)
    .filter(([key]) => key.startsWith('brand'))
    .map(([, value]) => value);
  if (colorValues.some((value) => typeof value !== 'string' || !/^#[0-9a-f]{6}$/i.test(value)))
    errors.push('THEME_COLOR_INVALID');
  const localeKeys = Object.keys(configuration.messages).sort();
  const sourceKeys = Object.keys(configuration.messages.en ?? {})
    .sort()
    .join('|');
  if (
    localeKeys.join('|') !== [...supportedLocales].sort().join('|') ||
    Object.values(configuration.messages).some(
      (catalog) =>
        Object.keys(catalog).sort().join('|') !== sourceKeys || Object.values(catalog).some((value) => !value.trim())
    )
  )
    errors.push('LOCALE_CATALOG_INVALID');
  const serialized = JSON.stringify(configuration);
  if (
    /"[^"\n]*(?:secret|password|token)[^"\n]*"\s*:/i.test(serialized) ||
    /-----BEGIN [A-Z ]+ PRIVATE KEY-----|\bsk-[A-Za-z0-9]{20,}/.test(serialized)
  )
    errors.push('PUBLIC_SECRET_INVALID');
