...
| WC-104 end-to-end Docker runner supply | IN-PROGRESS CHECKPOINT - Founder-authorized milestones through `28b62365` establish four canonical narrow runner identities, one serialized GHCR producer path, digest/provenance-verifying consumers, catalog-owned execution across local preparation/PR/main, and automatic per-node evidence reuse bound to base/head, changed files, catalog/gate/command implementation, exact runner OCI or tool digest, declared environment and evidence schema. Local Docker evidence passes the 80-test catalog, supply and reuse slice plus prior component suites. Remaining work includes hosted three-sample PR/main evidence, full gate equivalence, rollback rehearsal, exact ledger closure, author review, PR submission and Founder review/merge; no hosted or completion claim is made. |
