...
**State Schema:** 2.0.0
**State Revision:** 172
**Last Updated:** 2026-09-17 (WC-095 FAIL-CLOSED REPAIR CHECKPOINT)
...
| WC-095 agent employment lifecycle | PARTIAL / FAIL-CLOSED REPAIR CHECKPOINT - Milestone `5fb0f1e8` prevents goals alone from unlocking Operations, names relationship/onboarding/induction/Skill/goal/PR/commercial/contract/authority blockers, reserves a null API field for the complete Section 6.3 mandate, and adds exact-version customer goal verification through the server-only Portal BFF. Docker evidence passes 23 BP workspace tests, 12 focused Web tests, TypeScript, lint, production build, OpenAPI and generated-client checks. WC095-07 is `BLOCKED_CONTRACT` because no accepted BP-to-PR command carries the complete mandate; WC095-09 is `BLOCKED_CONTRACT` because no persisted seven-dimension review aggregate exists; WC095-05B remains `BLOCKED_EXTERNAL_INPUT`. WC095-10, full browser/accessibility, deployment, Production, PR approval and merge are not claimed. |
