...
| WC-104 end-to-end Docker runner supply | IN-PROGRESS CHECKPOINT - Founder-authorized milestones through `4305fb45` establish four canonical narrow runner identities, one serialized GHCR producer path, digest/provenance-verifying consumers, catalog-owned execution across local preparation/PR/main, automatic exact per-node evidence reuse and an executable frozen baseline for all authoritative gate bindings, external GitHub gates, thresholds, evidence classes and Shadow controls. Local Docker evidence passes the 150-test validation-control/equivalence slice plus prior component suites. Remaining work includes hosted three-sample PR/main evidence, rollback rehearsal, exact ledger closure, author review, PR submission and Founder review/merge; no hosted or completion claim is made. |
