...
| WC-105 authentication, Guide, session and landing defects | IN-PROGRESS CHECKPOINT - residual milestones `0d114ee4` and `b785f67b` add transaction-local tenant RLS with cancellation cleanup, map Demo Business Platform minimum replicas to one, and classify Facebook OAuth 191 without retaining provider details. Docker evidence passes the focused PostgreSQL contract and 75 affected infrastructure/verifier tests. Web, final evidence, exact-head PR preparation, hosted CI, Demo cold-start/provider acceptance and Founder review/merge remain open; no cloud or provider mutation occurred. |
