...
**State Schema:** 2.0.0
**State Revision:** 190
**Last Updated:** 2026-09-20 (WC-104 FULL DOCKER BUILD CHECKPOINT)
...
| WC-104 end-to-end Docker runner supply | IN-PROGRESS / PARTIALLY QUALIFIED - Founder-authorized milestones through `d4f327f4` establish four canonical narrow runner identities, one serialized GHCR producer path, digest/provenance-verifying consumers, catalog-owned execution across local preparation/PR/main, exact per-node evidence reuse, frozen authoritative gate equivalence and executable clean rollback. Local Docker control regression passes 99 validation-control tests plus 87 changed pipeline-control tests, and R014 isolation passes 39 tests. The exact `eb33e533` rollback clean-built all four runners once and attempted all 43 gates serially with no registry or prior-result reuse; 16 passed and 27 exposed PR-bound metadata, inherited repository/environment blockers or subsequently repaired orchestration defects. The validated 16-row ledger records nine local PASS rows, six hosted-evidence blockers and R016 blocked pending a passing clean rehearsal and dependency closure. The committed candidate passes the complete 16-image Compose build (211/211 BuildKit steps), and author review R-144 passes for `d4f327f4`. Hosted PR/main evidence, PR submission and Founder review/merge remain; no hosted or completion claim is made. |
