...
| WC-104 end-to-end Docker runner supply | IN-PROGRESS CHECKPOINT - Founder-authorized milestones through `5119ec02` establish four canonical narrow runner identities, one serialized GHCR producer path, digest/provenance-verifying consumers, catalog-owned execution across local preparation/PR/main, automatic exact per-node evidence reuse, an executable frozen baseline for authoritative gates and an executable clean rollback mechanism that disables registry and prior-result reuse, builds every runner once and traverses the full catalog serially. Local Docker evidence passes the 153-test validation-control/equivalence slice plus prior component suites. Remaining work includes hosted three-sample PR/main evidence, the exact-candidate rollback rehearsal, exact ledger closure, author review, PR submission and Founder review/merge; no hosted or completion claim is made. |
