...
**State Schema:** 2.0.0
**State Revision:** 162
**Last Updated:** 2026-09-15 (WC-089 DMA RELEASE 1 IMPLEMENTATION CHECKPOINT)
...
| WC-089 DMA Release 1 | IN-PROGRESS CHECKPOINT - Issue #437 and Founder session authority permit repository implementation, local candidate build, qualification evidence and an unmerged PR; recorded scope amendments permit the Release 1 AVD and additive owner contracts. Release `1.0.0` remains independent from specification `3.1`; build authority preflight, immutable `agentInstanceId` transport/isolation, canonical workload audience, real Skills 0/1/2, strict schemas, AVD, schema-valid content-bound admission snapshot and BP exact-three catalogue disclosure pass focused Docker tests. PR-owned append-only work revisions, WBE-owned usage attribution and BP read-only Work/Results projection now bind tenant, relationship, immutable instance, Skill/version and invocation; focused WBE 19/19, PR 8/8 and BP 19/19 tests pass, and the aligned BP OpenAPI has zero Spectral errors. The first local image digest is preliminary because image source changed afterward; exact-head rebuild/SBOM/provenance/conformance, remaining BP/WBE lifecycle closures, complete CCT/simulation coverage, final qualification and PR remain open. Provider, cloud, deployment, DNS, UAT, Production, self-approval and merge are prohibited. |
