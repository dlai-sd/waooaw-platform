...
| WC-089 DMA Release 1 | ENGINEERING QUALIFIED CHECKPOINT - Issue #437 and Founder session authority permitted repository implementation, local candidate build, qualification evidence and one unmerged PR; recorded scope amendments permitted the Release 1 AVD and additive owner contracts. Release `1.0.0` remains independent from specification `3.1`; build authority preflight, immutable `agentInstanceId` transport/isolation, canonical workload audience, real Skills 0/1/2, strict schemas, AVD, content-bound admission, BP exact-three catalogue, PR-owned append-only work, WBE-owned usage attribution and BP read-only Work/Results projection are implemented. Final local qualification passes 79 Python and 49 BP tests, Ruff, three OpenAPI specs, reproducible admitted image `sha256:085fe3121aeae81b3ce25b1c7a9dc1eec6437719b7115a052c5e54591d07bc7b`, in-image conformance, SBOM, HIGH/CRITICAL Trivy and diff-scoped gitleaks. PR submission, Founder review and merge remain; provider, cloud, deployment, DNS, UAT, Production and customer traffic were not tested and remain prohibited. |
