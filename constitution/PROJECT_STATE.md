...
| WC-089 DMA Release 1 | IN-PROGRESS CHECKPOINT - Issue #437 and Founder session authority permit repository implementation, one local candidate build, qualification evidence and an unmerged PR. Release `1.0.0` remains independent from specification `3.1`; build authority preflight, immutable `agentInstanceId` transport/isolation, canonical workload audience, real Skills 0/1/2, strict schemas and the Founder-authorized AVD pass focused Docker tests. Admission identity, exact candidate digest/SBOM/provenance/conformance, BP/WBE lifecycle closures, complete CCT/simulation coverage, final qualification and PR remain open. Provider, cloud, deployment, DNS, UAT, Production, self-approval and merge are prohibited. |
