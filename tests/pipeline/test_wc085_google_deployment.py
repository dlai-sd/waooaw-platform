...
def test_demo_seeder_provisions_dedicated_identity_reader_secret_at_runtime() -> None:
    ...
    identity_reader = next(
        entry for entry in catalog["entries"] if entry["vaultSecretName"] == "bp-identity-reader-client-secret"
    )
    assert identity_reader["provisioner"] == "environment-deployment"
    assert "goal006_deployment_credentials.py" in workflow
    assert "for name in $CREDENTIAL_NAMES; do" in workflow
