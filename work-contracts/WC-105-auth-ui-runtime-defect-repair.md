...
## Founder-Authorized Session Defect Tranche - 2026-09-24

The Founder authorized implementation and code changes for the defects below in the 2026-09-24
working session. Execution remains local and Codespaces-scoped: no Azure mutation, deployment,
provider mutation, Production action, customer traffic, self-approval, or self-merge is authorized.
Provider-backed acceptance may use an already configured provider callback but must not change that
provider configuration.

| Requirement | Required observable behavior | Executable evidence |
|---|---|---|
| WC105-R027 | One repository-owned script starts the complete WAOOAW customer portal stack on the current Codespace HTTPS origin in a repeatable fashion, prepares required local data and configuration, waits for readiness, and prints the review URL. Login, registration, Marketplace, My Agents, and customer portal routes use the started services rather than Demo dependencies or browser fixtures. A lightweight operator document makes this the standard response whenever the Founder asks for a Codespaces application URL. | Shell contract tests, Docker Compose configuration validation, real-container readiness, and browser smoke acceptance against the forwarded Codespaces URL. |
| WC105-R028 | A stale or revoked local/Keycloak session cannot trap a successful Facebook broker return on Login. Provider initiation clears only stale authentication state, a fresh provider result replaces it, and the customer proceeds to registration or the requested protected route. | NextAuth callback tests plus a browser journey beginning with an inactive session and completing Facebook-equivalent broker login. Real Facebook acceptance remains Founder-observed evidence. |
| WC105-R029 | Marketplace and My Agents either render successful real-service `200` projections or emit a privacy-safe correlation and typed failure. Broad page catches may not erase the owning failure, and production acceptance cannot rely on fixture-only service behavior. | Generated-client contract tests, real Business Platform browser journeys, and negative presentation assertions with retained reason codes. |
| WC105-R030 | Demo/local PostgreSQL does not use `POSTGRES_HOST_AUTH_METHOD=trust`; every database process requires an explicit secret-backed credential and remains unreachable outside its owning runtime network. | Rendered Compose/Terraform assertions plus real-container authenticated and unauthenticated connection checks. |
| WC105-R031 | Web-to-Business-Platform authentication and portal journeys emit privacy-safe correlated telemetry sufficient to distinguish provider, callback, session, API, and rendering failures. | Local collector span/log assertions and a retained end-to-end trace for login through Marketplace and My Agents. Azure ingestion remains outside this local authorization. |
| WC105-R032 | ASP.NET Data Protection keys persist across Business Platform container restart, are protected at rest, and do not invalidate applicable protected state during a routine restart. | Real-container restart test with pre/post protected-state verification and no ephemeral-key startup warning. |
| WC105-R033 | Container Apps scaling configuration references only valid targets and produces no `ScaledObjectCheckFailed: Target resource doesn't exist` event in deterministic validation. | Terraform/rendered-resource assertion and local Azure emulator lifecycle evidence. |
| WC105-R034 | `/professionals/mine` has one route owner and one test owner; duplicate route-group implementations cannot diverge or mask which customer experience is deployed. | Route-manifest uniqueness test and focused My Agents browser acceptance. |
| WC105-R035 | Requesting Login with an existing session whose identity validation is unavailable must display the Login UI instead of redirecting into a protected-route error loop. Starting a provider login from that UI clears stale local state before OAuth. | Login-view unavailable-session regression test plus a browser journey from `/home` to the intercepted Login dialog. |
| WC105-R036 | A Google or Facebook token accepted by the local preview JWT boundary must progress to registration-required or a ready customer session without requiring unavailable private Keycloak Admin credentials. This local claims-proof mode must be explicit and impossible outside the local environment. | Signed-JWT Program-host test with real PostgreSQL, non-local denial test, and deployed provider launch acceptance. |
| WC105-R037 | Codespaces preview sessions expire after one hour. Every deliberate launcher deployment creates a new authority generation and rejects JWT authority from an earlier deployment, while an ordinary container restart retains the active generation. | NextAuth unit tests, launcher contract tests, and a fresh-deployment browser session check. |
| WC105-R038 | The preview supplies a valid 256-bit continuity-envelope HMAC key so relationship controllers construct successfully and empty customer relationship lists return without `500`. | Launcher contract, key-strength assertion, focused relationship tests, and rebuilt-preview logs. |
| WC105-R039 | The isolated preview Marketplace contains the current active DMA offer for existing and newly registered active local customer workspaces without weakening production admission filtering. | Preview seed contract, idempotent PostgreSQL fixture check, and authenticated Marketplace acceptance. |
| WC105-R040 | Every Business Platform deployment supplies the Constitutional Engine endpoint through the exact `ConstitutionalEngine:GrpcUrl` key consumed by the gateway. Hire and Trial must reach constitutional authorization before persistence and cannot leave partial relationships on denial or dependency failure. | Compose/Terraform configuration contracts, acquisition controller tests, CE request logs, and relationship-row checks. |
| WC105-R041 | Selecting Google launches the configured `keycloak-google` broker directly with explicit account selection and no intermediate WAOOAW Login caused by a pre-launch local sign-out. | Provider command component tests and rebuilt-preview browser acceptance. |
| WC105-R042 | An already authenticated customer who accepts Hire or Trial disclosure continues directly to Marketplace acquisition without routing through or flashing Registration. | Disclosure continuation component tests and rebuilt-preview browser acceptance. |
| WC105-R043 | Sign out exposes pending and failed states, revokes backend authority, and clears local NextAuth state. Switch account clears both authority layers before launching Google account selection and must not present `Access not permitted` as successful completion. | Sign-out and account-switch success, fallback, failure, and ordering component tests plus rebuilt-preview browser acceptance. |
| WC105-R044 | A protected tab that regains focus or visibility revalidates backend session authority and clears stale local UI on `401/403`. A dependency `5xx` remains truthful and does not falsely log the customer out; unresolved identities do not mount the guard. | Session validity and protected-shell component tests plus a rebuilt-preview two-tab lifecycle check. |
| WC105-R045 | PostgreSQL tenant scoping must not alter EF/Npgsql command batches or their affected-row accounting. Multi-entity tenant-scoped writes execute under a transaction-local tenant value without false concurrency failures. | Real PostgreSQL interceptor regression test and deployed Hire relationship persistence. |
| WC105-R046 | Marketplace offers Trial only when both real trial-owner integrations are configured. An unsupported direct Trial continuation fails before constitutional evidence or relationship persistence; Hire remains available. | Marketplace projection and acquisition controller tests plus rebuilt-preview browser acceptance and relationship-row checks. |
| WC105-R047 | Customer relationship detail, timeline, workspace, evaluation, and conversation routes resolve membership through the customer identity boundary instead of denying valid customer JWTs because route metadata is absent. | Controller metadata, focused Business Platform tests, and rebuilt-preview relationship-detail request logs. |
| WC105-R048 | Acquisition failure UI never claims that nothing started or that retry is safe when the server outcome may be partial. A known non-retryable response directs the customer to My Agents; only a transient `503` may retry the identical idempotency key. | Acquisition continuation component tests and rebuilt-preview browser acceptance. |
| WC105-R049 | Every My Agents card prominently identifies its authoritative `HIRE` or `TRIAL` acquisition mode independently of lifecycle state. New acquisitions persist the selected mode, legacy trial bindings backfill as Trial, and ambiguous legacy rows remain explicitly unrecorded rather than inferred. | PostgreSQL migration result, acquisition/controller tests, My Agents component tests, production build, and rebuilt-preview data verification. |

These requirements are not complete from source-shaped tests alone. R027 must pass first and becomes
the execution environment for R028, R029, R031, R032, and R034. Each later requirement must bind its
evidence to the exact local image digest or source revision used by the printed Codespaces URL.