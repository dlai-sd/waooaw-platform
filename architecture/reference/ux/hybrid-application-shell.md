...
| Durable conversation timeline, send, retry, acknowledgement, read position, cancellation, and text-only message/card schemas | Business Platform public API; Professional Runtime supplies internal execution/session outcomes | WC-034 F3 review candidate in `components/conversation-core.md` and Business Platform OpenAPI 1.2.0 | BLOCKED pending generated-client validation and independent INST-004 approval; attachments remain deferred |
| Professional response stream | Business Platform public SSE boundary; Professional Runtime owns the internal execution SSE | WC-034 F3 review candidate in BP OpenAPI 1.2.0 and PR OpenAPI 1.1.0 | BLOCKED pending independent INST-004 approval; ordinary browser traffic must not connect directly to PR |
