...
4. The GitHub-hosted job validates `architecture/reference/pipeline/github-runner-app-manifest.json` against the live organization installation before every use. GitHub enforces the organization self-hosted-runner permission server-side. The App has no repository contents write, Actions workflow write, environment approval, PR approval, package mutation, administration, or unrelated organization permission; any manifest mismatch or additional live permission blocks token issuance. INST-007 owns manifest changes and INST-009 owns deterministic validation.
5. The GitHub App RSA private key is imported as a non-exportable Key Vault key, not stored as a secret. The bootstrap OIDC identity receives `Key Vault Crypto User` on that exact key version and asks Key Vault to sign the GitHub App JWT with RS256; private key bytes never enter workflow or runner memory. The identity can `set/delete` only an environment-specific short-lived runner-token secret and has no list-all-secrets or environment deployment role. It exchanges the signed JWT for one GitHub runner registration token and writes only that token to the short-lived secret with a 15-minute expiry. The ACA runner identity can `get` only the short-lived token secret and has no key-sign permission. Bootstrap deletes the token secret as soon as the runner is online and always by expiry cleanup.
6. The workflow has three jobs: `bootstrap-runner` on GitHub-hosted compute reconciles/validates the stack, obtains the token and starts ACA; `deploy-private` targets the environment runner label; `cleanup-runner` runs on GitHub-hosted compute with `if: always()` and depends on both preceding jobs. ACA enforces a 60-minute execution timeout and GitHub ephemeral mode accepts one deployment job. The cleanup job stops the ACA execution and deletes/disables any remaining GitHub runner after normal completion, failure, cancellation or timeout. Because a hard workflow termination cannot guarantee downstream execution, the Deployment Stack also owns a separate ACA scheduled `runner-reconciler` Job. It runs every five minutes for at most two minutes with a distinct cleanup identity, Key Vault sign access to the non-exportable App key, GitHub runner administration, and ACA execution read/stop authority; it has no runner-token read, state/config data access, or environment deployment role. It removes stale registrations/token secrets/executions and records evidence. Cleanup verifies deregistration and zero active ACA executions within five minutes, retries for at most 15 minutes, then keeps the environment label inactive and notifies INST-009/007. No next deployment starts while an orphan remains; manual recovery requires INST-007 confirmation that the runner and token secret are absent.
...
The approved DNS parameter manifest is a preventive pre-start gate. The bootstrap job runs Deployment Stack `what-if`, compares live links/endpoints/records with the manifest, and stops before token creation or ACA execution on any mismatch. It does not auto-remediate unexpected cross-environment links. The failed reconciliation, live inventory and alert are retained in the run artifact for INST-009/007 review.
