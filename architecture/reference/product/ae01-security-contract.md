...
Any authenticated participant may request Stop when the relationship is resolvable; fail-safe Stop takes priority. Release is limited to an active same-tenant participant bound as `EMPLOYER`, not merely the stopper, evaluator, relationship manager, channel possessor, or platform operator. Release requires Tier-4 portal reauthentication no older than five minutes, display of originating Stop reason/time/scope, the literal `RELEASE_EMERGENCY_STOP` confirmation, a non-empty justification, and both the originating Stop evidence ID and correlation. BP validates that the evidence is the active Stop for this tenant and relationship, then writes release evidence whose constitutional basis includes `EMERGENCY_STOP_RELEASE:<originating-stop-evidence-id>` before the internal transition command may leave `STOPPED_EMERGENCY`. Missing, mismatched, erased-payload, or unavailable proof fails closed and leaves the relationship stopped. No conversational “resume,” reconnect, timeout, retry, session token refresh, or platform operator can release it.
