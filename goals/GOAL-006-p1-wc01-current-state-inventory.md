...
| Rollback and verification | Blue-green deployment script contains traffic rollback on failed green health checks. Post-deploy workflow declares a rollback job. | `scripts/blue-green-deploy.sh`; `.github/workflows/environment-deployment-verification.yaml` | `REPOSITORY_VERIFIED` | Post-deploy rollback execution is a TODO and only records `PENDING`; no general release rollback workflow is implemented. |
