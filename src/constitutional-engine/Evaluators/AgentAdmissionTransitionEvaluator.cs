...
{
    private static readonly IReadOnlyDictionary<string, string[]> Authorities = new Dictionary<
        string,
        string[]
    ...
    )
    {
        ...
        try
        {
            ...
            if (
                !DigestPattern().IsMatch(contentDigest)
                || !DigestPattern().IsMatch(evidenceDigest)
                || string.IsNullOrWhiteSpace(policyVersion)
                || !authorities.Contains(authority, StringComparer.Ordinal)
            )
                return Deny("Admission transition authority or immutable binding is invalid.");
            if (
                context.ActionType != "AGENT_ADMISSION_SUBMIT"
                && submitter is not null
                && string.Equals(actor, submitter, StringComparison.OrdinalIgnoreCase)
            )
                return Deny(
                    "Admission submitter cannot perform an independent lifecycle transition."
                );
