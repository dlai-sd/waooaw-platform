...
public sealed record EvaluationContextItem(
...
public sealed record EvaluationSkillItem(
...
public sealed record EvaluationDecisionSpace(
    int Version,
    long BudgetCeilingInrPaise,
    JsonElement AuthorityBoundaries,
    JsonElement StopConditions,
    int ReviewCadenceMonths
);
...
) : ControllerBase
{
    ...
    )
    {
        ...
        var context = contextRows
            .Select(item => new EvaluationContextItem(
                item.PayloadReference,
                item.FieldType,
                Parse(item.ValueJson!),
                item.ConfirmationStatus
            ))
            .ToList();
        var fields = context.Select(item => item.FieldType).ToHashSet(StringComparer.Ordinal);
        var nextQuestion =
            !fields.Contains("NAME") ? "What name should this professional use for your business?"
        ...
        var trial = trialRow is { TrialId: not null, StartsAt: not null, ExpiresAt: not null }
            ? new EvaluationTrial(
                trialRow.TrialId.Value,
                trialRow.StartsAt.Value,
                trialRow.ExpiresAt.Value,
                trialRow.Status
            )
            : null;
        var goals = await db
            ...
            .OrderBy(item => item.CreatedAt)
            .Select(item => new EvaluationGoalItem(
                item.GoalId,
                item.Goal,
                item.Measure,
                item.Status,
                item.ReviewCadenceMonths
            ))
            .ToListAsync(cancellationToken);
        var skills = await db
            ...
            .Where(item => item.TenantId == tenantId && item.RelationshipId == relationshipId)
            .OrderBy(item => item.CreatedAt)
            .Select(item => new EvaluationSkillItem(
                item.ConfigurationId,
                item.SkillId,
                item.SkillVersion,
                $"skill-{item.UpdatedAt.UtcTicks}",
                item.Applicability,
            ...
            .ToListAsync(cancellationToken);
        var decisionRow = await db
            .DecisionSpaceSnapshots.AsNoTracking()
            .Where(item => item.TenantId == tenantId && item.RelationshipId == relationshipId)
            .OrderByDescending(item => item.Version)
            .FirstOrDefaultAsync(cancellationToken);
        var decision = decisionRow is null
            ? null
            : new EvaluationDecisionSpace(
                decisionRow.Version,
        ...
        return Ok(
            new RelationshipEvaluationProjection(
                relationshipId,
                RelationshipStateCodec.ToDatabase(relationship.State),
                relationship.State is EmploymentRelationshipState.Discovered
                    ? "NOT_STARTED"
                    : "AVAILABLE",
                context,
                nextQuestion,
