...
public sealed record CustomerAlertMutationRequest(
    string SchemaVersion,
    string ExpectedAlertVersion
);
...
public sealed class NotificationsController(CustomerAlertService alerts) : ControllerBase
{
    ...
    public Task<IActionResult> MarkReadAsync(
        Guid alertId,
        [FromBody] CustomerAlertMutationRequest request,
    ...
    private async Task<IActionResult> MutateAsync(
        Guid alertId,
        CustomerAlertMutationRequest request,
    ...
    )
    {
        ...
        if (
            ...
            || string.IsNullOrWhiteSpace(request.ExpectedAlertVersion)
        ...
        try
        {
            return Ok(
                ToResponse(
                    await alerts.SetReadStateAsync(
                        tenantId,
                        alertId,
                        actor,
                        parsedKey,
                        operation,
                        request.ExpectedAlertVersion,
                        hash,
                        targetState,
                        cancellationToken
                    )
    ...
    private static object ToResponse(CustomerAlert alert) =>
        new
        {
            ...
            destination = new
            {
                surface = alert.DestinationSurface,
                relationshipId = alert.RelationshipId,
                subjectId = alert.DestinationSubjectId,
            },
