...
public sealed record PutAgentAdmissionRevisionRequest(
    int ExpectedStateVersion,
    string AdmissionContentDigest,
    JsonElement AdmissionContent
);

public sealed record ValidateAgentAdmissionRequest(
    int Revision,
    string AdmissionContentDigest,
...
public sealed class AgentAdmissionsController(AgentAdmissionService admissions) : ControllerBase
{
    ...
    {
        if (environment is not ("demo" or "uat" or "prod"))
            return AdmissionProblem(400, "ADMISSION_INVALID_REQUEST");
    ...
    {
        if (!TryContext(out var tenantId, out var actorId))
            return AdmissionProblem(401, "ADMISSION_UNAUTHORIZED");
        if (idempotencyKey is null)
            return AdmissionProblem(400, "ADMISSION_INVALID_REQUEST");
        if (request.OwnerSubjectId != actorId && !IsRole("admission_operator"))
            return Forbid();
    ...
    )
    {
        ...
        return await ExecuteAsync(async () =>
        {
            var result = await admissions.ValidateAsync(
                ...
                request.Revision,
                request.AdmissionContentDigest,
                request.ValidatorProfile,
                actorId,
                IsRole("validator_operator"),
                idempotencyKey.Value,
                cancellationToken
            );
            var response = new
    ...
    [Authorize]
    [HttpGet(
        "{type}/versions/{version}/admission/drafts/{draftId:guid}/validations/{validationId:guid}/findings"
    )]
    ...
    )
    {
        ...
        return await ExecuteAsync(async () =>
            Ok(
                await admissions.GetFindingsAsync(
                    tenantId,
                    type,
                    version,
                    draftId,
    ...
    public Task<IActionResult> SupersedeAsync(
        string type,
        string version,
        [FromBody] AgentAdmissionTransitionRequest request,
        [FromHeader(Name = "Idempotency-Key")] Guid? key,
    ...
    public Task<IActionResult> RetireAsync(
        string type,
        string version,
        [FromBody] AgentAdmissionTransitionRequest request,
        [FromHeader(Name = "Idempotency-Key")] Guid? key,
    ...
    )
    {
        ...
        return await ExecuteAsync(async () =>
        {
            ...
            return StatusCode(
                operation == "SUBMIT" && !result.Replayed ? 201 : 200,
                Projection(result.Admission)
            );
        });
    }
    ...
    private async Task<IActionResult> ExecuteAsync(Func<Task<IActionResult>> action)
    {
        ...
        catch (AdmissionIdempotencyConflictException)
        {
            return AdmissionProblem(409, "ADMISSION_IDEMPOTENCY_CONFLICT");
        }
        catch (AdmissionStateConflictException)
        {
            return AdmissionProblem(409, "ADMISSION_STATE_CONFLICT");
        }
        catch (AdmissionTransitionBlockedException)
        {
            return AdmissionProblem(423, "ADMISSION_TRANSITION_BLOCKED");
        }
    ...
    private bool AuthorizedFor(string operation) =>
        operation switch
        {
    ...
    private static object Projection(AgentAdmission admission) =>
        new
        {
