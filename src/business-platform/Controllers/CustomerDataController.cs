...
public sealed class CustomerDataController : ControllerBase
{
    ...
    )
    {
        ...
        {
            _logger.LogWarning(
                "EraseCustomerData rejected: caller lacks founder role. TenantId={TenantId}",
                tenantId
            );
            return Forbid();
        }
        ...
        var payloads = await db
            .OperationalPayloads.Where(p => p.TenantId == tenantId && p.ErasedAt == null)
        ...
        var ceAddress =
            _config["ConstitutionalEngine:GrpcAddress"] ?? "http://constitutional-engine:7000";
        try
        {
