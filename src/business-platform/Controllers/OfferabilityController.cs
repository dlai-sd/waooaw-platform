...
public sealed class OfferabilityController(
    ...
    OfferabilityOrchestrationService orchestration
) : ControllerBase
{
    ...
    {
        if (!IsFounder() || !TryContext(out var tenantId, out var participantId))
            return Forbid();
        ...
        if (relationship is null)
            return Problem(404, "OFFERABILITY_NOT_ACCESSIBLE");
        try
        {
            var decision = await orchestration.EvaluateAsync(
                new OfferabilityEvaluationRequest(
                    tenantId,
                    relationshipId,
            ...
            return Ok(
                new
                {
                    ...
                    ownerVersions = JsonSerializer.Deserialize<JsonElement>(
                        decision.OwnerVersionsJson
                    ),
        ...
        catch (OfferabilityIdempotencyConflictException)
        {
            return Problem(409, "OFFERABILITY_IDEMPOTENCY_CONFLICT");
        }
        catch (ConstitutionalActionDeniedException)
        {
            return Problem(423, "OFFERABILITY_BLOCKED");
        }
    ...
    private bool IsFounder() =>
        User.IsInRole("founder")
        || string.Equals(
            User.FindFirstValue("participant_role"),
            "FOUNDER",
    ...
    private ObjectResult Problem(int status, string code) =>
        StatusCode(
            status,
            new
            {
