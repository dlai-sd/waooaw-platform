...
public class RenewalFailureSaga
{
    ...
    public async Task RunAsync(RenewalFailureInput input)
    {
        ...
        await Workflow.ExecuteActivityAsync(
            (RenewalFailureActivities a) =>
                a.SendPaymentFailureAlertAsync(input.CustomerId, input.ContractId),
            ActivityOpts
        );
        ...
        await Workflow.ExecuteActivityAsync(
            (RenewalFailureActivities a) =>
                a.SetDegradedModeAsync(input.CustomerId, input.ContractId, input.AgentType),
            ActivityOpts
        );
        ...
        await Workflow.ExecuteActivityAsync(
            (RenewalFailureActivities a) =>
                a.PauseCampaignsAsync(input.CustomerId, input.AgentType),
            ActivityOpts
        );
        await Workflow.ExecuteActivityAsync(
            (RenewalFailureActivities a) =>
                a.SuspendContractAsync(input.CustomerId, input.ContractId),
            ActivityOpts
        );
        ...
        await Workflow.ExecuteActivityAsync(
            (RenewalFailureActivities a) =>
                a.TerminateContractAsync(input.CustomerId, input.ContractId),
            ActivityOpts
        );
    }
...
public class RenewalFailureActivities(IHttpClientFactory httpClientFactory)
{
    ...
    public async Task SetDegradedModeAsync(string customerId, string contractId, string agentType)
    {
        ...
        await _paas.PostAsJsonAsync(
            $"/api/v1/sessions/{customerId}/mode",
            new
            {
                mode = "DEGRADED",
    ...
    public async Task SuspendContractAsync(string customerId, string contractId)
    {
        ...
        await _paas.PostAsJsonAsync(
            $"/api/v1/contracts/{contractId}/suspend",
            new { reason = "RENEWAL_FAILURE_DAY7" }
        );
    }
    ...
    public async Task TerminateContractAsync(string customerId, string contractId)
    {
        ...
        await _paas.PostAsJsonAsync(
            ...
            new { reason = "RENEWAL_FAILURE_DAY14" }
        );
    }
}
