...
{
    internal VerifiedCustomerActor(string issuer, string subject) =>
...
{
    internal VerifiedGoogleWorkspaceProof(
...
public sealed record CustomerWorkspaceTrust(
    string ActorIssuer,
    string ProviderIssuer,
    string BrokerAlias,
    string TrustConfigDigest
...
public sealed record CustomerWorkspaceMembership(
    Guid AccountId,
    Guid TenantId,
    Guid MembershipId,
    string[] Roles
...
public sealed record CustomerWorkspaceCompletion(
    IdentityCompletionResult Result,
    int StatusCode,
    string ResponseBody
);
...
public sealed class CustomerWorkspaceException(
    CustomerWorkspaceError error,
...
public sealed class CustomerWorkspaceProvisioningService
{
    ...
    public CustomerWorkspaceProvisioningService(
        IDbContextFactory<IdentityDbContext> factory,
        CustomerWorkspaceTrust trust,
    ...
    )
    {
        ...
        if (
            !ValidKey(trust.ActorIssuer, 256)
            || !ValidKey(trust.ProviderIssuer, 256)
            || !ValidKey(trust.BrokerAlias, 40)
            || !HashPattern.IsMatch(trust.TrustConfigDigest)
        )
            throw new ArgumentException(
                "Explicit verified broker trust configuration is required.",
                nameof(trust)
            );
    }

    public async Task<CustomerWorkspaceCompletion> CompleteAsync(
        VerifiedGoogleWorkspaceProof proof,
        Guid registrationId,
        Guid idempotencyKey,
    ...
    )
    {
        ...
        if (
            ...
            || !HashPattern.IsMatch(canonicalHash)
        )
                ...
                var retryable =
                    ...
                    || databaseError.SqlState == PostgresErrorCodes.UniqueViolation
                        && databaseError.ConstraintName
                            is "actor_bindings_actor_unique"
                                or "login_methods_provider_unique"
                                or "idempotency_ledger_actor_key_op_unique";
                ...
                throw new CustomerWorkspaceException(
                    databaseError.SqlState == PostgresErrorCodes.UniqueViolation
                        ...
                        : databaseError.SqlState
                            ...
                            ? CustomerWorkspaceError.InvariantViolation
                        : CustomerWorkspaceError.DependencyUnavailable,
                    exception
                );
            }
    ...
    public async Task<CustomerWorkspaceMembership> ResolveAsync(
        VerifiedCustomerActor actor,
        CancellationToken ct = default
    )
    {
    ...
    public async Task<CustomerWorkspaceMembership> ResolveInTransactionAsync(
    ...
    )
    {
        ...
        if (db.Database.CurrentTransaction is null)
            throw new InvalidOperationException(
                "Membership resolution requires an existing transaction."
            );
        try
        {
            ...
            {
                var actorExists = await db.ActorBindings.AnyAsync(
                    binding =>
                        binding.ActorIssuer == actor.Issuer
                        && binding.ActorSubject == actor.Subject,
                    ct
                );
                throw new CustomerWorkspaceException(
                    actorExists
            ...
            await db.Database.ExecuteSqlInterpolatedAsync(
                $"""
                SELECT pg_catalog.set_config('app.tenant_id', {membership.TenantId.ToString(
                    "D"
                )}, true),
                       pg_catalog.set_config('app.current_tenant_id', {membership.TenantId.ToString(
                    "D"
                )}, true)
                """,
                ct
    ...
    private async Task<CustomerWorkspaceCompletion> CompleteAttemptAsync(
        VerifiedGoogleWorkspaceProof proof,
        Guid registrationId,
        Guid idempotencyKey,
        string canonicalHash,
    ...
    )
    {
        ...
        var replay = await db.IdempotencyLedger.SingleOrDefaultAsync(
            entry =>
                entry.ActorIssuer == proof.Actor.Issuer
                && entry.ActorSubject == proof.Actor.Subject
                && entry.OperationFamily == OperationFamily
                && entry.IdempotencyKey == keyText,
            ct
        );
        if (
            replay is not null
            && (replay.CanonicalHash != canonicalHash || replay.RegistrationId != registrationId)
        )
        ...
        var binding = await db.ActorBindings.SingleOrDefaultAsync(
            actor =>
                actor.ActorIssuer == proof.Actor.Issuer
                && actor.ActorSubject == proof.Actor.Subject,
            ct
        );
        ...
        {
            var login = await db.LoginMethods.SingleOrDefaultAsync(
                record => record.LoginMethodId == binding.LoginMethodId,
                ct
            );
            if (
                ...
                || login is null
                || login.Status != "ACTIVE"
                || login.AccountId != binding.AccountId
                || login.ProviderIssuer != proof.ProviderIssuer
                || login.BrokerAlias != proof.BrokerAlias
                || login.ProviderSubject != proof.ProviderSubject
            )
                throw new CustomerWorkspaceException(CustomerWorkspaceError.RecoveryRequired);
            var live =
                await ReadMembershipAsync(db, ct)
                ?? throw new CustomerWorkspaceException(CustomerWorkspaceError.MembershipRequired);
            ...
            var account = await db.Accounts.SingleAsync(
                record => record.AccountId == binding.AccountId,
                ct
            );
            root = await db.Registrations.SingleAsync(
                record => record.RegistrationId == account.OriginRegistrationId,
                ct
            );
        ...
        if (registration.State != IdentityRegistrationState.Completed)
        {
            ...
            db.RegistrationEvents.Add(
                new IdentityRegistrationEventRecord
                {
                    RegistrationId = registrationId,
                    ActorIssuer = proof.Actor.Issuer,
                    ActorSubject = proof.Actor.Subject,
        ...
        if (
            registration.AccountId != root.AccountId
            || registration.ActorBindingId != root.ActorBindingId
            || registration.OriginRegistrationId != root.RegistrationId
            || registration.CompletionResponseBody != root.CompletionResponseBody
            || registration.CompletionOutcome != root.CompletionOutcome
            || registration.CompletionProfileSnapshot != root.CompletionProfileSnapshot
            || registration.CompletionStatusCode != root.CompletionStatusCode
        ...
        if (
            replay is not null
            && (
                replay.StatusCode != root.CompletionStatusCode
                || replay.ResponseBody != root.CompletionResponseBody
            )
        )
            throw new CustomerWorkspaceException(CustomerWorkspaceError.InvariantViolation);
        if (replay is null)
        ...
        if (membership.AccountId != root.AccountId)
            throw new CustomerWorkspaceException(CustomerWorkspaceError.InvariantViolation);
        var result = JsonSerializer.Deserialize<IdentityCompletionResult>(
            root.CompletionResponseBody!,
            JsonOptions
        );
        if (
            result is null
            || result.AccountReference != root.AccountId
            || result.Outcome != root.CompletionOutcome
            || result.AssuranceLevel != "AAL2_ACCOUNT"
            || result.DefaultTarget != "APPLICATION_HOME"
        )
            throw new CustomerWorkspaceException(CustomerWorkspaceError.InvariantViolation);
    ...
    private async Task<IdentityActorBindingRecord> CreateCohortAsync(
        IdentityDbContext db,
        VerifiedGoogleWorkspaceProof proof,
    ...
    )
    {
        ...
        while (tenantId == accountId)
            tenantId = Guid.NewGuid();
        var binding = new IdentityActorBindingRecord
        {
            ActorBindingId = Guid.NewGuid(),
            ActorIssuer = proof.Actor.Issuer,
            ActorSubject = proof.Actor.Subject,
            LoginMethodId = Guid.NewGuid(),
            AccountId = accountId,
            VerifiedAt = proof.VerifiedAt,
            AuthTime = proof.AuthTime,
            TrustConfigDigest = proof.TrustConfigDigest,
            CorrelationId = proof.CorrelationId,
        };
        db.ActorBindings.Add(binding);
        await db.SaveChangesAsync(ct);
        db.LoginMethods.Add(
            new IdentityLoginMethodRecord
            {
                LoginMethodId = binding.LoginMethodId,
                AccountId = accountId,
                ProviderIssuer = proof.ProviderIssuer,
                BrokerAlias = proof.BrokerAlias,
        ...
        db.Accounts.Add(
            new IdentityAccountRecord
            {
                ...
                OriginRegistrationId = registration.RegistrationId,
            }
        );
        await db.SaveChangesAsync(ct);
        var acceptedDomain = await db
        ...
        db.Organisations.Add(
            new IdentityOrganisationRecord
            {
                Id = tenantId,
                TenantId = tenantId,
        ...
        registration.CompletionProfileSnapshot = JsonSerializer.Serialize(
            new
            {
                registration.DisplayName,
                registration.BusinessName,
                registration.BusinessDomain,
                registration.LanguagePreference,
                emailVerified = true,
    ...
    {
        if (
            db.Database.GetDbConnection() is not NpgsqlConnection connection
            || new NpgsqlConnectionStringBuilder(connection.ConnectionString).NoResetOnClose
        )
            throw new InvalidOperationException(
                "A reset-enabled PostgreSQL connection is required."
            );
        await db.Database.ExecuteSqlInterpolatedAsync(
            $"""
    ...
    private static async Task<CustomerWorkspaceMembership?> ReadMembershipAsync(
        IdentityDbContext db,
        CancellationToken ct
    )
    {
        ...
        command.CommandText =
            "SELECT account_id, tenant_id, membership_id, roles FROM identity.resolve_customer_membership()";
        await using var reader = await command.ExecuteReaderAsync(ct);
        if (!await reader.ReadAsync(ct))
            return null;
        var result = new CustomerWorkspaceMembership(
            reader.GetGuid(0),
            reader.GetGuid(1),
            reader.GetGuid(2),
        ...
        if (
            ...
            || result.AccountId == result.TenantId
            || !result.Roles.SequenceEqual(["OWNER"])
        )
            throw new CustomerWorkspaceException(CustomerWorkspaceError.InvariantViolation);
        return result;
    }
    ...
    private void ValidateActor(VerifiedCustomerActor actor)
    {
        if (
            actor is null
            || actor.Issuer != _trust.ActorIssuer
            || !ValidKey(actor.Issuer, 256)
            || !ValidKey(actor.Subject, 256)
        )
    ...
    private void ValidateProof(VerifiedGoogleWorkspaceProof proof)
    {
        ...
        if (
            proof.ProviderIssuer != _trust.ProviderIssuer
            || proof.BrokerAlias != _trust.BrokerAlias
            || proof.TrustConfigDigest != _trust.TrustConfigDigest
            || !ValidKey(proof.ProviderSubject, 256)
            || proof.CorrelationId == Guid.Empty
        )
            throw new CustomerWorkspaceException(CustomerWorkspaceError.ProofRequired);
        var now = _time.GetUtcNow();
        if (
            proof.AuthTime < now - Freshness
            || proof.VerifiedAt < now - Freshness
            || proof.AuthTime > now + ClockSkew
            || proof.VerifiedAt > now + ClockSkew
            || proof.AuthTime > proof.VerifiedAt + ClockSkew
        )
            throw new CustomerWorkspaceException(
                CustomerWorkspaceError.FreshAuthenticationRequired
            );
    ...
    private void ValidateRegistration(IdentityRegistrationRecord registration, string brokerAlias)
    {
        ...
        if (
            registration.AuthenticationPath != expectedPath
            || !registration.EmailVerified
            || !ValidProfile(registration.DisplayName, 120)
            || !ValidProfile(registration.BusinessName, 160)
            || !ValidProfile(registration.BusinessDomain, 100)
            || registration.LanguagePreference is null
            || !LanguagePattern.IsMatch(registration.LanguagePreference)
            || registration.State != IdentityRegistrationState.Completed
                && (
                    registration.State != IdentityRegistrationState.ReadyToComplete
                    || registration.ExpiresAt <= _time.GetUtcNow()
                )
        )
            throw new CustomerWorkspaceException(CustomerWorkspaceError.RegistrationIneligible);
    }

    private static void ValidateCompletedRoot(
        IdentityRegistrationRecord root,
        IdentityActorBindingRecord binding
    )
    {
        if (
            root.State != IdentityRegistrationState.Completed
            || root.AccountId != binding.AccountId
            || root.ActorBindingId != binding.ActorBindingId
            || root.ActorIssuer != binding.ActorIssuer
            || root.ActorSubject != binding.ActorSubject
            || root.OriginRegistrationId != root.RegistrationId
            || root.CompletedAt is null
            || root.CompletionStatusCode != 200
            || root.CompletionOutcome is not ("ACCOUNT_CREATED" or "ACCOUNT_REUSED")
            || root.CompletionProfileSnapshot is null
            || root.CompletionResponseBody is null
        )
            throw new CustomerWorkspaceException(CustomerWorkspaceError.InvariantViolation);
    }

    private static bool ValidProfile(string? value, int maximum) =>
        !string.IsNullOrWhiteSpace(value) && ValidKey(value, maximum);
    ...
    private static long LockKey(string[] parts) =>
        BinaryPrimitives.ReadInt64BigEndian(
            SHA256
                .HashData(JsonSerializer.SerializeToUtf8Bytes(parts, LockJsonOptions))
                .AsSpan(0, 8)
        );
