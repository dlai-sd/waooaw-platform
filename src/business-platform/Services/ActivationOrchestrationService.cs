...
public sealed class ActivationConflictException()
    : Exception("Canonical activation tuple has divergent material.");

public sealed class ActivationOwnerUnavailableException(
    string reason,
...
{
    private static readonly ConcurrentDictionary<string, CanonicalTupleLock> CanonicalTupleLocks =
        new();
    ...
    {
        var tupleKey =
            $"{request.TenantId:D}|{request.RelationshipId:D}|{request.AcceptedContractId:D}|{request.CommercialOutcomeKind}|{request.PaymentReference}";
    ...
    )
    {
        ...
        if (
            !CryptographicOperations.FixedTimeEquals(
                Convert.FromHexString(existing.MaterialRequestHash),
    ...
    private static CanonicalTupleLock AcquireTupleLockReference(string tupleKey)
    {
        while (true)
        {
            ...
            {
                if (
                    CanonicalTupleLocks.TryGetValue(tupleKey, out var current)
    ...
    private static void ReleaseTupleLockReference(string tupleKey, CanonicalTupleLock tupleLock)
    {
        lock (tupleLock)
        {
            ...
            if (tupleLock.References == 0)
                CanonicalTupleLocks.TryRemove(
                    new KeyValuePair<string, CanonicalTupleLock>(tupleKey, tupleLock)
                );
        }
    }
    ...
    )
    {
        ...
        try
        {
            ...
            if (billingOutcome.Status != "ACTIVE" || billingOutcome.SubscriptionId == Guid.Empty)
                throw new ActivationOwnerUnavailableException(
                    "WBE commercial activation outcome is not active."
                );
        }
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            ...
            throw exception is ActivationOwnerUnavailableException
                ? exception
                : new ActivationOwnerUnavailableException(
                    "WBE commercial activation outcome is unresolved.",
                    exception
                );
        }
        ...
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            await MarkRetryableAsync(intent.ActivationIntentId, cancellationToken);
            throw new ActivationOwnerUnavailableException(
                "Constitutional activation evidence is unresolved.",
                exception
            );
        }
    ...
    private async Task<string> ValidateEligibilityAsync(
        ActivationRequest request,
        CancellationToken cancellationToken
    )
    {
        ...
        if (
            relationship.AcceptedContractId != request.AcceptedContractId
            ...
            throw new ActivationEligibilityException(
                "Relationship is not eligible for paid activation."
            );
        ...
        if (!accepted)
            throw new ActivationEligibilityException("Exact contract acceptance is not eligible.");
        await offerabilityGuard.RequireEligibleAsync(
            request.TenantId,
            request.RelationshipId,
            cancellationToken
        );
    ...
    private async Task EnterActivationPendingAsync(
        ActivationRequest request,
        CancellationToken cancellationToken
    )
    {
        var relationship = await relationships.GetAsync(
            request.TenantId,
            request.RelationshipId,
            cancellationToken
        );
        ...
        if (relationship?.State != EmploymentRelationshipState.ContractAcceptedPendingPayment)
            throw new ActivationEligibilityException(
                "Relationship cannot enter activation pending."
            );
    ...
    )
    {
        ...
        var intent = await db.ActivationIntents.SingleAsync(
            value => value.ActivationIntentId == intentId,
            cancellationToken
        );
        if (intent.Status == "SUCCEEDED")
            return ToOutcome(intent);
        ...
        if (relationship.State != EmploymentRelationshipState.ActivationPending)
            throw new ActivationEligibilityException(
                "Relationship left activation pending before completion."
            );
        ...
        intent.OutcomeJson = JsonSerializer.Serialize(
            new { subscription_id = subscriptionId, relationship_state = "ACTIVE" }
        );
    ...
    private async Task MarkRetryableAsync(Guid intentId, CancellationToken cancellationToken)
    {
        ...
        if (intent.Status is "SUCCEEDED" or "CONFLICT")
            return;
    ...
    )
    {
        ...
        var stored = await db.ActivationIntents.SingleAsync(
            value => value.ActivationIntentId == intent.ActivationIntentId,
            cancellationToken
        );
            ...
            SHA256.HashData(
                Encoding.UTF8.GetBytes(
                    string.Join(
                        ...
                        request.ContractVersion,
                        request.ContractAcceptanceId.ToString("D"),
                        request.PaymentReference,
                        request.PaymentEvidenceId.ToString("D"),
                        request.CommercialOutcomeKind,
    ...
    private static ActivationOutcome ToOutcome(ActivationIntent intent) =>
        new(
            intent.ActivationIntentId,
            intent.OutcomeSubscriptionId
                ?? throw new ActivationOwnerUnavailableException(
                    "Stored subscription outcome is absent."
                ),
            intent.OutcomeEvidenceId
                ?? throw new ActivationOwnerUnavailableException(
                    "Stored evidence outcome is absent."
                ),
            intent.Status
        );
}
...
public sealed class UnconfiguredActivationBillingGateway : IActivationBillingGateway
{
    public Task<ActivationBillingOutcome> ActivatePaidSubscriptionAsync(
        ...
        throw new ActivationOwnerUnavailableException(
            "Authenticated WBE paid activation is not configured."
        );
}
