...
{
    Task<ActivationOutcome> StartOrJoinAsync(
        ActivationRequest request,
        CancellationToken cancellationToken
    );
}

public sealed class TemporalActivationWorkflowStarter(ITemporalClient temporalClient)
    : IActivationWorkflowStarter
{
    ...
    )
    {
        ...
        {
            return await temporalClient
                .GetWorkflowHandle(workflowId)
                .GetResultAsync<ActivationOutcome>();
        }
    }
}
...
)
{
    ...
    )
    {
        ...
        if (
            request.CommercialOutcomeKind is not ("CAPTURED" or "ZERO_PRICE_SATISFIED")
        ...
        if (
            relationship.State
                is not (
                    ...
                    or EmploymentRelationshipState.Active
                )
        ...
        if (!employer)
            throw new ConstitutionalActionDeniedException(
                "Activation requires an active same-tenant EMPLOYER binding."
            );
        var acceptance =
            await db
            ...
            ?? throw new ActivationEligibilityException(
                "Exact contract acceptance is required for activation."
            );
        ...
        var activation = new ActivationRequest(
            ...
            StableCorrelation(
                ...
                request.CommercialOutcomeReference
            ),
            request.CommercialOutcomeKind
        );
