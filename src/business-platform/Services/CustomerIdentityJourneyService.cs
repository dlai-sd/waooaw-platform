...
public sealed class CustomerIdentityJourneyService(
    IdentityService identity,
    IDbContextFactory<IdentityDbContext> factory,
    GoogleWorkspaceProofAdapter proofAdapter,
...
)
{
    ...
    public async Task<(IdentityRegistrationRecord reg, bool isNew)> StartAsync(
    ...
    )
    {
        ...
        return await identity.StartRegistrationAsync(
            proofAdapter.ValidateActor(principal, requireFresh: true),
            authenticationPath,
            key,
            CanonicalHash("StartRegistration", null, new { languagePreference = language }),
            language,
            verifiedEmail is not null,
            verifiedEmail is null ? null : IdentityService.MaskEmail(verifiedEmail),
            ct
        );
    }

    public Task<IdentityRegistrationRecord> GetAsync(
        ClaimsPrincipal principal,
        Guid registrationId,
        CancellationToken ct
    ...
    public Task<(
        ...
        identity.StartEmailVerificationAsync(
            registrationId,
            ValidateActor(principal),
            key,
            canonicalHash,
    ...
    public Task<(
        ...
        identity.StartMobileVerificationAsync(
            registrationId,
            ValidateActor(principal),
            key,
            CanonicalHash("StartMobileVerification", registrationId, new { mobile }),
            mobile,
            ct
        );

    public Task<(IdentityRegistrationRecord reg, bool isNew)> UpdateAsync(
        ClaimsPrincipal principal,
        Guid registrationId,
        Guid key,
        string displayName,
        ...
        identity.UpdateProfileAsync(
            registrationId,
            ValidateActor(principal),
            proofAdapter.AuthenticationPath(principal),
            key,
            CanonicalHash(
                "UpdateProfile",
                registrationId,
    ...
    )
    {
        ...
        return await Provisioning(proofAdapter.TrustFor(principal))
            .CompleteAsync(
                proof,
                registrationId,
                key,
                CanonicalHash("CompleteRegistration", registrationId, new { }),
                ct
            );
    }

    public Task<CustomerWorkspaceMembership> ResolveAsync(
        ClaimsPrincipal principal,
        CancellationToken ct
    )
    {
        ...
        return Provisioning(proofAdapter.TrustFor(principal))
            .ResolveAsync(ValidateActor(principal), ct);
    }

    private CustomerWorkspaceProvisioningService Provisioning(CustomerWorkspaceTrust trust) =>
        new(factory, trust);

    private static string ProviderId(IdentityAuthenticationPath authenticationPath) =>
        authenticationPath switch
        {
