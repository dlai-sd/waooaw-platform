...
public sealed record OfferableSkill(
    string SkillId,
    string SkillVersion,
    string Capability,
    string BusinessKpi
...
)
{
    ...
    {
        var requestHash = AgentAdmissionCanonicalizer.Digest(
            $"CREATE|{professionalType}|{professionalVersion}|{ownerSubjectId:D}"
        );
        await using var db = await dbFactory.CreateDbContextAsync(cancellationToken);
        var prior = await db
            .AgentAdmissionIdempotency.AsNoTracking()
        ...
        {
            if (
                prior.ActorSubjectId != actorSubjectId
                || prior.MaterialRequestHash != requestHash
                || prior.Status != "COMPLETED"
            )
                throw new AdmissionIdempotencyConflictException();
            var replayed =
                await db.AgentAdmissions.SingleOrDefaultAsync(
        ...
        db.AgentAdmissionIdempotency.Add(
    ...
    )
    {
        ...
        var requestHash = AgentAdmissionCanonicalizer.Digest(
            $"REVISE|{revision}|{expectedStateVersion}|{actualDigest}"
        );
        if (
            await FindReplayAsync(
                db,
                admission,
                "REVISE",
                idempotencyKey,
        ...
        if (
            admission.StateVersion != expectedStateVersion
            || revision != admission.CurrentRevision + 1
            || admission.State
                is not (
                    AgentAdmissionState.Draft
                    or AgentAdmissionState.RemediationRequired
                    or AgentAdmissionState.Rejected
                )
        )
            throw new AdmissionStateConflictException();

        var predecessor = await db
        ...
        db.AgentAdmissionRevisions.Add(
            new AgentAdmissionRevision
            {
                TenantId = tenantId,
    ...
    {
        if (profile != AgentAdmissionValidator.Profile)
            throw new ArgumentException("Unsupported validator profile.");
        await using var db = await dbFactory.CreateDbContextAsync(cancellationToken);
        var admission = await RequireAsync(
            db,
            tenantId,
        ...
        if (!validatorOperator)
            RequireOwner(admission, actorSubjectId);
        var requestHash = AgentAdmissionCanonicalizer.Digest(
            $"VALIDATE|{revision}|{contentDigest}|{profile}"
        );
        var existing = await db
            .AgentAdmissionValidations.AsNoTracking()
            .SingleOrDefaultAsync(
        ...
        if (
            admission.State
                is not (AgentAdmissionState.Draft or AgentAdmissionState.RemediationRequired)
            || admission.CurrentRevision != revision
            || admission.AdmissionContentDigest != contentDigest
        )
            throw new AdmissionStateConflictException();
        ...
        db.AgentAdmissionFindings.AddRange(
            findings.Select(finding => new AgentAdmissionFinding
            {
                TenantId = tenantId,
        ...
        admission.State =
            findings.Count == 0
                ? AgentAdmissionState.Validated
                : AgentAdmissionState.RemediationRequired;
        admission.StateVersion++;
        admission.UpdatedAt = DateTimeOffset.UtcNow;
        await SaveOrConflictAsync(db, cancellationToken);
    ...
    {
        if (
            !RequiredReadiness.Contains(observation.AssertionType, StringComparer.Ordinal)
            || observation.Environment is not ("demo" or "uat" or "prod")
            || observation.Status
                is not ("PASS" or "FAIL" or "UNKNOWN" or "UNAVAILABLE" or "REVOKED")
        ...
        await using var db = await dbFactory.CreateDbContextAsync(cancellationToken);
        var admission =
            await FindAsync(db, tenantId, professionalType, professionalVersion, cancellationToken)
            ?? throw new AdmissionNotFoundException();
        if (
            admission.State
            is not (
                AgentAdmissionState.Approved
                or AgentAdmissionState.Active
                or AgentAdmissionState.Suspended
            )
        )
            throw new AdmissionTransitionBlockedException(
                "Readiness can bind only an approved admission revision."
            );
        var expectedDigest =
            observation.AssertionType == "ARTIFACT"
                ? admission.ArtifactDigest
                : admission.AdmissionContentDigest;
        if (
            expectedDigest != observation.SubjectDigest
            || admission.PolicyVersion != observation.PolicyVersion
        )
            throw new AdmissionTransitionBlockedException(
                "Readiness subject or policy does not match the approved admission."
            );
        db.AgentAdmissionAssertions.Add(
            new AgentAdmissionAssertion
            {
                TenantId = tenantId,
    ...
    )
    {
        ...
        db.AgentAdmissionOutbox.Add(
            new AgentAdmissionOutbox
            {
                TenantId = tenantId,
                AdmissionId = admission.AdmissionId,
                TransitionId = transition.TransitionId,
                EventType = $"agent.admission.{operation.ToLowerInvariant()}",
                ScopeHash = AgentAdmissionCanonicalizer.Digest(
                    $"{tenantId:D}|{admission.AdmissionId:D}|{transition.TransitionId:D}|{target}"
                ),
    ...
    )
    {
        ...
        command.CommandText =
            "SELECT projection::text FROM business.get_offerable_professional_versions(@environment)";
        ...
        {
            var projection =
                JsonSerializer.Deserialize<OfferableProfessionalVersion>(
                    reader.GetString(0),
    ...
    {
        if (admission.OwnerSubjectId != actorSubjectId)
            throw new AdmissionNotFoundException();
    }
    ...
    )
    {
        var replay = await db
            ...
            .SingleOrDefaultAsync(
                value =>
                    value.TenantId == admission.TenantId
        ...
        if (replay is null)
            return false;
        if (
            replay.AdmissionId != admission.AdmissionId
            || replay.ActorSubjectId != actorSubjectId
            || replay.MaterialRequestHash != requestHash
    ...
    private static string TransitionRequestHash(
        string operation,
        AdmissionTransitionIntent intent,
        string environment
    ) =>
        AgentAdmissionCanonicalizer.Digest(
    ...
    )
    {
        ...
        if (
            ...
            throw new AdmissionTransitionBlockedException(
                "Review must bind the exact submitted evidence, artifact, and policy."
            );
        if (
            operation == "ACTIVATE"
            && (
            ...
            throw new AdmissionTransitionBlockedException(
                "Activation must bind the exact approved artifact and policy."
            );
        if (operation != "SUBMIT" && admission.SubmitterSubjectId == actorSubjectId)
            throw new AdmissionTransitionBlockedException(
                "Submitter cannot perform an independent lifecycle transition."
            );
    ...
    private static AgentAdmissionState TargetState(string operation) =>
        operation switch
        {
    ...
    {
        var current = await CurrentAssertionsAsync(
            db,
            admission,
            environment,
        ...
        if (!RequiredReadiness.All(type => current.Contains(type)))
            throw new AdmissionTransitionBlockedException(
                "All exact current readiness assertions are required."
            );
        if (string.IsNullOrWhiteSpace(intent.ArtifactDigest))
            throw new AdmissionTransitionBlockedException(
                "Artifact digest is required for activation."
            );
    }
    ...
    {
        var rows = await db
            .AgentAdmissionAssertions.AsNoTracking()
            .Where(value =>
                value.TenantId == admission.TenantId
                && value.AdmissionId == admission.AdmissionId
                && value.Environment == environment
                && value.PolicyVersion == admission.PolicyVersion
    ...
    private static async Task SaveOrConflictAsync(
        EmploymentRelationshipDbContext db,
        CancellationToken cancellationToken
    )
    {
