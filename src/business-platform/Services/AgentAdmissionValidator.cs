...
public static class AgentAdmissionCanonicalizer
{
    ...
    {
        if ((BitConverter.DoubleToInt64Bits(number) & long.MaxValue) == 0)
            return "0";
...
public sealed class AgentAdmissionValidator
{
    ...
    )
    {
        ...
        if (content.ValueKind != JsonValueKind.Object)
        {
            Add(
                ...
                "/",
                "C-088, C-094",
                "admission contract JSON object",
        ...
        if (
            String(content, "contractSchemaVersion") != "1.0.0"
            || String(content, "canonicalizationProfile") != "RFC8785"
        )
            Add(
                findings,
                "AAV-002",
        ...
        if (
            !string.Equals(
                AgentAdmissionCanonicalizer.Digest(content),
                suppliedDigest,
            ...
            Add(
                ...
                "/",
                "C-059, C-063",
                "RFC 8785 SHA-256 digest matching immutable content",
                "DIGEST_MISMATCH"
            );
        if (
            identity is null
            ...
            Add(
                ...
                "/professionalIdentity",
                "C-059, C-063",
                "path-bound professional type and version",
                "IDENTITY_MISMATCH"
            );

        if (
            identity is null
            || !HasObjects(identity.Value, "agentSpecification", "agentVerificationDocument")
            || compliance is null
            || !HasObjects(compliance.Value, "constitutionalDna", "agentBaseSpec")
        )
            Add(
                findings,
                "AAV-004",
                "/professionalIdentity",
                "C-070, C-094",
                "exact specification, AVD, DNA, and Base Spec references",
        ...
        if (compliance is null || Object(compliance.Value, "platformAgentContract") is null)
            Add(
                findings,
                "AAV-005",
                "/complianceDeclaration/platformAgentContract",
                "C-041, C-094",
                "exact compatible PAC declaration",
                "PAC_INCOMPLETE"
            );
        ...
        if (skills is null || skills.Value.GetArrayLength() == 0)
        {
            Add(
                ...
                "/skillManifest",
                "C-036, C-037",
                "one or more complete Skill Definitions",
                "SKILLS_MISSING"
            );
        }
        else
        ...
        if (
            compliance is null
            || !HasNonEmptyArray(compliance.Value, "evidenceFirstOperations")
            ...
            Add(
                ...
                "/complianceDeclaration",
                "C-023, C-059, C-099",
                "DCM, Evidence First operations, CCTs, and artifact bindings",
                "CONFORMANCE_INCOMPLETE"
            );
        if (
            compliance is null
            || Object(compliance.Value, "emergencyStop") is null
            ...
            Add(
                ...
                "/complianceDeclaration",
                "C-001, C-049, C-079",
                "consistent retention, degradation, and Emergency Stop policies",
                "POLICY_INCOMPLETE"
            );
        if (includeReadiness)
            Add(
                ...
                "/activationEvidence",
                "C-063, C-079",
                "current platform-owned readiness assertions",
                "READINESS_REQUIRED"
            );
    ...
    )
    {
        ...
        if (compliance is { } declaration)
        {
            Known(
                ...
                "evidenceFirstOperations",
                "emergencyStop",
                "constitutionalConformanceTests",
                "dataClasses",
                "retentionPolicy",
                "securityPosture",
                "limitationBehavior",
            ...
            KnownChild(
                declaration,
                "emergencyStop",
                "/complianceDeclaration/emergencyStop",
                findings,
                "supported",
                "signal",
            ...
            foreach (
                var name in new[] { "constitutionalDna", "agentBaseSpec", "decisionConsequenceMap" }
            )
                KnownChild(
                    declaration,
                    name,
                    $"/complianceDeclaration/{name}",
                    findings,
                    "path",
                    "version",
                    "digest"
        ...
        if (identity is { } identityValue)
        {
            KnownChild(
                identityValue,
                "agentSpecification",
                "/professionalIdentity/agentSpecification",
                findings,
                "path",
                "version",
                "digest"
            ...
            KnownChild(
                identityValue,
                "agentVerificationDocument",
                "/professionalIdentity/agentVerificationDocument",
                findings,
                "path",
                "version",
                "digest"
        ...
        if (skills is not { } manifest)
            return;
        foreach (
            var (skill, index) in manifest.EnumerateArray().Select((value, index) => (value, index))
        )
        {
            ...
            KnownChild(
                ...
                $"{path}/reviewPolicy",
                findings,
                "performanceReviewDays",
                "customerContractReviewDays"
            );
            KnownChild(
                skill,
                "nonGoalExemption",
                $"{path}/nonGoalExemption",
                findings,
                "exemptionId",
                "purpose",
                "scope",
                "measurableOperationalOutcome",
                "approvingAuthority",
            ...
            if (Array(skill, "tools") is { } tools)
                foreach (
                    var (tool, toolIndex) in tools
                        .EnumerateArray()
                        .Select((value, toolIndex) => (value, toolIndex))
                )
                    Known(
                        tool,
    ...
    {
        if (Object(parent, name) is { } child)
            Known(child, path, findings, allowed);
    }
    ...
    )
    {
        ...
        foreach (
            ...
            Add(
                findings,
                "AAV-001",
                $"{path.TrimEnd('/')}/{property.Name}",
                "C-088, C-094",
                "field declared by contract schema",
                "UNKNOWN_FIELD"
            );
    }
    ...
    )
    {
        var path = $"/skillManifest/{index}";
        if (
            string.IsNullOrWhiteSpace(String(skill, "skillId"))
            ...
            Add(
                ...
                path,
                "C-036, C-037",
                "skill KPI, Decision Space, and admitted schemas",
                "SKILL_INCOMPLETE"
            );
        ...
        if (
            ...
            Add(
                ...
                $"{path}/tools",
                "C-003, C-041",
                "declared default-denied tools mapped to governed actions",
                "TOOL_AUTHORITY_INCOMPLETE"
            );
        if (Object(skill, "costUnits") is null)
            Add(
                ...
                $"{path}/costUnits",
                "C-038, C-088",
                "digest-bound billing cost mapping",
                "COST_MAPPING_MISSING"
            );
        if (
            Object(skill, "schedulePolicy") is null
            || Object(skill, "reviewPolicy") is null
            ...
            Add(
                ...
                path,
                "C-001, C-049, C-079",
                "cadence, review, trial, and degradation policies",
                "SKILL_POLICY_INCOMPLETE"
            );
    }
