...
public sealed class OperationalMandateResolver(
    IDbContextFactory<EmploymentRelationshipDbContext> relationshipFactory) : IOperationalMandateResolver
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var relationship = await db.EmploymentRelationships.AsNoTracking().SingleOrDefaultAsync(
            ...
            cancellationToken);
        var participantExists = await db.RelationshipParticipants.AsNoTracking().AnyAsync(
        ...
        if (relationship is null || !participantExists)
            return new OperationalMandateReadiness(false, ["The relationship mandate identity is unavailable."]);
        ...
        var admission = await db.AgentAdmissions.AsNoTracking().SingleOrDefaultAsync(
            value => value.TenantId == tenantId
        ...
        else if (!await db.AgentAdmissionRevisions.AsNoTracking().AnyAsync(
            value => value.TenantId == tenantId && value.AdmissionId == admission.AdmissionId
                && value.Revision == admission.CurrentRevision, cancellationToken))
            blocked.Add("The current admission revision is unavailable.");

        if (!await db.ContractAcceptances.AsNoTracking().AnyAsync(
            value => value.TenantId == tenantId && value.RelationshipId == relationshipId
                && value.ContractId == relationship.AcceptedContractId!.Value, cancellationToken))
            blocked.Add("The accepted contract evidence is unavailable.");
        if (!await db.DecisionSpaceSnapshots.AsNoTracking().AnyAsync(
            value => value.TenantId == tenantId
                && value.SnapshotId == relationship.AuthoritySnapshotId!.Value, cancellationToken))
        ...
        foreach (var skill in skills)
        {
            ...
            var bindingReady = admission is not null && await db.AgentSkillRuntimeBindings.AsNoTracking().AnyAsync(
                value => value.TenantId == tenantId && value.AdmissionId == admission.AdmissionId
                    && value.SkillId == skill.SkillId && value.SkillVersion == skill.SkillVersion
                    && value.SupersededAt == null, cancellationToken);
            if (!bindingReady)
    ...
    {
        await using var db = await relationshipFactory.CreateDbContextAsync(cancellationToken);
        var replay = await db.OperationalMandateSnapshots.AsNoTracking().SingleOrDefaultAsync(
            value => value.TenantId == tenantId
                && value.RelationshipId == relationshipId
        ...
        {
            var replayMandate = JsonSerializer.Deserialize<OperationalMandateV1>(replay.MandateJson, JsonOptions)
                ?? throw new OperationalMandateUnavailableException();
            if (replayMandate.ActorId != participantId
                || !string.Equals(replayMandate.SkillId, skillId, StringComparison.Ordinal)
                || !string.Equals(replayMandate.OperationalPurpose, operationalPurpose, StringComparison.Ordinal))
            {
        ...
        var participant = await db.RelationshipParticipants.AsNoTracking().SingleOrDefaultAsync(
            value => value.TenantId == tenantId
                ...
                && value.Status == "ACTIVE",
            cancellationToken);
        if (relationship is null || participant is null
            || relationship.State is not (EmploymentRelationshipState.Active or EmploymentRelationshipState.TrialActive)
        ...
        var admissionRevision = await db.AgentAdmissionRevisions.AsNoTracking().SingleOrDefaultAsync(
            value => value.TenantId == tenantId
                && value.AdmissionId == admission.AdmissionId
                && value.Revision == admission.CurrentRevision,
            cancellationToken);
        var runtimeBinding = await db.AgentSkillRuntimeBindings.AsNoTracking().SingleOrDefaultAsync(
            value => value.TenantId == tenantId
                && value.AdmissionId == admission.AdmissionId
                && value.SkillId == skillId
                && value.SupersededAt == null,
            cancellationToken);
        var skill = await db.RelationshipSkillConfigurations.AsNoTracking().SingleOrDefaultAsync(
            value => value.TenantId == tenantId
                && value.RelationshipId == relationshipId
                && value.SkillId == skillId
                && value.Status == "ACCEPTED",
            cancellationToken);
        if (admissionRevision is null || runtimeBinding is null || skill is null
        ...
        var goal = await db.RelationshipGoals.AsNoTracking().SingleOrDefaultAsync(
            value => value.TenantId == tenantId
                && value.RelationshipId == relationshipId
                && value.GoalId == skill.GoalId.Value
                && value.Status == "ACTIVE",
            cancellationToken);
        var goalDecision = await db.RelationshipGoalDecisions.AsNoTracking()
        ...
        var contextRevision = await db.ContextConfirmationEvents.AsNoTracking().CountAsync(
            value => value.TenantId == tenantId
                && value.RelationshipId == relationshipId
                && value.Action == "CONFIRMED",
            cancellationToken);
        var configurationRevision = await db.RelationshipSkillDecisions.AsNoTracking().CountAsync(
            value => value.TenantId == tenantId && value.RelationshipId == relationshipId,
            cancellationToken);
        var goalRevision = await db.RelationshipGoalDecisions.AsNoTracking().CountAsync(
            value => value.TenantId == tenantId && value.RelationshipId == relationshipId && value.GoalId == skill.GoalId,
            cancellationToken);
        var decisionSpace = await db.DecisionSpaceSnapshots.AsNoTracking().SingleOrDefaultAsync(
            value => value.TenantId == tenantId && value.SnapshotId == relationship.AuthoritySnapshotId.Value,
            cancellationToken);
        var acceptance = await db.ContractAcceptances.AsNoTracking().SingleOrDefaultAsync(
            ...
            cancellationToken);
        if (goal is null || goalDecision?.Decision != "VERIFIED" || goalDecision.SkillId != skillId
            || contextRevision == 0 || configurationRevision == 0 || goalRevision == 0
            || decisionSpace is null || acceptance is null)
        {
            throw new OperationalMandateUnavailableException();
        }
        ...
        var admittedSkill = root.GetProperty("skillManifest").EnumerateArray()
            .SingleOrDefault(value => value.GetProperty("skillId").GetString() == skillId
        ...
        deadline = new DateTimeOffset(deadline.Year, deadline.Month, deadline.Day, deadline.Hour, deadline.Minute, deadline.Second, TimeSpan.Zero);
        var permittedActions = admittedSkill.GetProperty("constitutionalActions").EnumerateArray()
            .Select(value => value.GetString()!).ToArray();
        var draft = new OperationalMandateV1(
            "1.0", mandateId, string.Empty, tenantId, relationshipId, relationship.AgentInstanceId,
            participantId, RelationshipRoleCodec.ToDatabase(participant.Role),
            ...
            admission.ProfessionalTypeId, runtimeBinding.ReleaseSequence, admission.ProfessionalVersion,
            runtimeBinding.SpecificationRevision, runtimeBinding.SpecificationDigest, admission.CurrentRevision,
            admission.AdmissionContentDigest, admission.ArtifactDigest,
            ...
            compliance.GetProperty("platformAgentContract").GetProperty("schemaVersion").GetString()!,
            adapter.GetProperty("protocolVersion").GetString()!, acceptance.ContractHash,
            skill.SkillId, skill.SkillVersion,
            runtimeBinding.InputSchemaDigest, runtimeBinding.OutputSchemaDigest,
            runtimeBinding.PromptVersion, runtimeBinding.PromptDigest,
            contextRevision, configurationRevision, goalRevision, decisionSpace.Version,
            $"decision-space:{decisionSpace.SnapshotId:D}:budget", admission.CurrentRevision,
            [$"contract-acceptance:{acceptance.AcceptanceId:D}", $"goal-decision:{goalDecision.DecisionId:D}"],
            false, null, operationalPurpose, permittedActions, ["UNDECLARED_ACTION"], deadline,
            idempotencyKey, $"ce-decision:{constitutionalEvidenceId:D}",
            $"ce-evidence:{constitutionalEvidenceId:D}", null, null);
        var mandate = draft with { MandateDigest = Digest(draft) };
        var mandateJson = JsonSerializer.Serialize(mandate, JsonOptions);
        db.OperationalMandateSnapshots.Add(new OperationalMandateSnapshot
        {
            MandateId = mandateId,
            TenantId = tenantId,
    ...
    {
        var canonical = JsonSerializer.Serialize(mandate with { MandateDigest = string.Empty }, JsonOptions);
        var normalized = JsonNode.Parse(canonical)?.AsObject()
            ?? throw new OperationalMandateUnavailableException();
        ...
        {
            builder.Append(separator).Append(JsonSerializer.Serialize(property.Name)).Append(':').Append(property.Value.GetRawText());
            separator = ",";
        }
        builder.Append('}');
