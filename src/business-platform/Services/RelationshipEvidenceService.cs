...
public sealed class GrpcRelationshipEvidenceGateway(IConfiguration configuration)
    : IRelationshipEvidenceGateway
{
...
public sealed class RelationshipEvidenceService
{
    ...
    )
    {
        if (string.IsNullOrWhiteSpace(purpose) || purpose.Length > 200)
            throw new ArgumentException(
                "Export purpose must contain 1 to 200 characters.",
                nameof(purpose)
            );
        ...
        var replay = await replayDb
            .RelationshipEvidenceExports.AsNoTracking()
        ...
        if (replay is not null)
        {
            if (
                ...
                throw new ChannelContinuityConflictException(
                    "Evidence export idempotency key conflicts."
                );
        ...
        var exportEvidenceId = await _constitutionalGateway.AuthorizeAndRecordAsync(
            ...
            new
            {
                ...
                participant_role = RelationshipRoleCodec.ToDatabase(role),
    ...
    private RelationshipEvidenceExportResult ToExportResult(
        RelationshipEvidenceExport export,
        bool replayed
    )
    {
        ...
        if (signingKey.Length < 32)
            throw new InvalidOperationException(
                "Evidence export signing key must contain at least 32 characters."
            );
        var material =
            $"{export.TenantId:D}:{export.RelationshipId:D}:{export.ParticipantId:D}:{export.ParticipantRole}:{export.ExportId:D}:{export.ExpiresAt.ToUnixTimeSeconds()}";
        var signature = Convert.ToHexStringLower(
            HMACSHA256.HashData(signingKey, Encoding.UTF8.GetBytes(material))
        );
        ...
        if (
            ...
            throw new InvalidOperationException(
                "Evidence export download base URL must use HTTPS."
            );
        var url =
            $"{baseUrl}/api/v1/evidence-exports/{export.ExportId:D}/download?expires={export.ExpiresAt.ToUnixTimeSeconds()}&signature={signature}";
        return new(
            export.ExportId,
            "COMPLETED",
            export.CreatedAt,
    ...
    )
    {
        ...
        return participant?.Role
            ?? throw new KeyNotFoundException("Relationship evidence is not accessible.");
    }
    ...
    )
    {
        ...
        ids.UnionWith(
            await db
                ...
                .Select(value => value.AcceptanceEvidenceId)
                .ToListAsync(cancellationToken)
        );
        ids.UnionWith(
            await db
                .ActivationIntents.AsNoTracking()
                .Where(value =>
                    value.TenantId == tenantId
                    && value.RelationshipId == relationshipId
                    && value.OutcomeEvidenceId.HasValue
                )
                .Select(value => value.OutcomeEvidenceId!.Value)
                .ToListAsync(cancellationToken)
        );
        var bindingEvidence = await db
            .ChannelBindings.AsNoTracking()
            .Where(value => value.TenantId == tenantId && value.RelationshipId == relationshipId)
            .Select(value => new
            {
                value.PreparedEvidenceId,
                value.BoundEvidenceId,
                value.RevokedEvidenceId,
            })
            .ToListAsync(cancellationToken);
        ids.UnionWith(bindingEvidence.Select(value => value.PreparedEvidenceId));
        ids.UnionWith(
            bindingEvidence
                .Where(value => value.BoundEvidenceId.HasValue)
                .Select(value => value.BoundEvidenceId!.Value)
        );
        ids.UnionWith(
            bindingEvidence
                .Where(value => value.RevokedEvidenceId.HasValue)
        ...
        var checkpointEvidence = await db
            .ContinuityCheckpoints.AsNoTracking()
            .Where(value => value.TenantId == tenantId && value.RelationshipId == relationshipId)
            .Select(value => new { value.PreparedEvidenceId, value.ResolutionEvidenceId })
            .ToListAsync(cancellationToken);
        ids.UnionWith(checkpointEvidence.Select(value => value.PreparedEvidenceId));
        ids.UnionWith(
            checkpointEvidence
                .Where(value => value.ResolutionEvidenceId.HasValue)
                .Select(value => value.ResolutionEvidenceId!.Value)
        );
        ids.UnionWith(
            await db
                .DeliveryAcknowledgements.AsNoTracking()
                .Where(value =>
                    value.TenantId == tenantId && value.RelationshipId == relationshipId
                )
                .Select(value => value.EvidenceId)
    ...
    private static bool IsVisible(RelationshipParticipantRole role, string actionType) =>
        role switch
        {
            RelationshipParticipantRole.Employer => true,
            RelationshipParticipantRole.Evaluator => ContainsAny(
                actionType,
                "DISCLOS",
                "TRIAL",
                "CONFIG",
            ...
            RelationshipParticipantRole.RelationshipManager => !ContainsAny(
                actionType,
                "PAYMENT",
                "CHARGE",
                "PERSONAL",
    ...
    private static RelationshipEvidenceDetail ToDetail(CustomerVisibleEvidenceRecord value)
    {
        ...
        return new RelationshipEvidenceDetail(
            ...
            "RECORDED",
            erased ? "ERASED"
                : value.HasPayloadRefId ? "AVAILABLE"
                : "NOT_RETAINED",
