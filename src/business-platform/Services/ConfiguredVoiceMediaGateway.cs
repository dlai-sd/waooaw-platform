...
public sealed class ConfiguredVoiceMediaGateway(IConfiguration configuration) : IVoiceMediaGateway
{
    ...
    {
        if (!AllowedTypes.Contains(declaredMediaType))
        ...
        if (
            !AllowedTypes.Contains(probe.MediaType)
        ...
        using (var aes = new AesGcm(_key, tag.Length))
        {
            aes.Encrypt(
                ...
                ciphertext,
                tag,
                Binding(tenantId, relationshipId, sessionId)
            );
        }
    ...
    public Task EraseAsync(string payloadReference, CancellationToken cancellationToken)
    {
        ...
        if (!Guid.TryParseExact(payloadReference, "N", out _))
    ...
    public Task PurgeExpiredAsync(DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (File.GetLastWriteTimeUtc(path) <= now.UtcDateTime)
                File.Delete(path);
        }
        return Task.CompletedTask;
    }
    ...
    private async Task<MediaProbe> InspectAsync(byte[] bytes, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var codec =
                document.RootElement.GetProperty("streams")[0].GetProperty("codec_name").GetString()
                ?? string.Empty;
            if (
                !decimal.TryParse(
                    format.GetProperty("duration").GetString(),
            ...
            {
                (var name, "opus") when name.Contains("webm", StringComparison.Ordinal) =>
                    "audio/webm",
                (var name, "opus") when name.Contains("ogg", StringComparison.Ordinal) =>
                    "audio/ogg",
                (var name, "pcm_s16le" or "pcm_s24le" or "pcm_s32le")
                    when name.Contains("wav", StringComparison.Ordinal) => "audio/wav",
    ...
    private async Task ScanAsync(byte[] bytes, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (result.Contains("FOUND", StringComparison.Ordinal))
                throw new VoiceBlockedException("quarantined");
    ...
    private string PayloadPath(string payloadReference) =>
        Path.Combine(_payloadRoot, $"{payloadReference}.voice");
    ...
    private static byte[] ReadKey(IConfiguration configuration)
    {
        ...
        return key.Length == 32
            ? key
            : throw new InvalidOperationException("Voice media key must be 256 bits.");
    }

    private sealed record MediaProbe(string MediaType, int DurationMilliseconds);
}

public sealed class VoiceMediaRetentionWorker(ConfiguredVoiceMediaGateway gateway)
    : BackgroundService
{
