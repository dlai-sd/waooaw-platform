...
)
{
    ...
    )
    {
        ...
        var skillExists = await db
            ...
            .AnyAsync(
                value =>
                    ...
                    && value.SkillVersion == skillVersion
        ...
        var revision =
            await db
                .PerformanceReviewWindows.Where(value =>
                    value.TenantId == tenantId
                    && value.RelationshipId == relationshipId
                    && value.SkillId == skillId
                )
                .Select(value => (int?)value.Revision)
        ...
        if (
            ...
            is not ("CONTINUE_CURRENT_MANDATE" or "TUNE_NON_MATERIAL_PRESENTATION")
        )
        {
            db.CustomerAlerts.Add(
                new CustomerAlert
                {
                    TenantId = tenantId,
                    AlertType = "PERFORMANCE_REVIEW",
                    Severity = draft.Recommendation
                        is "PAUSE_AFFECTED_WORK"
                            or "ESCALATE_LIMITATION_OR_BLOCKER"
                        ? "HIGH"
                        : "MEDIUM",
    ...
    {
        if (
            !CustomerDecisions.Contains(decision)
            || expectedReviewRevision < 1
            || idempotencyKey == Guid.Empty
            || correlationId == Guid.Empty
            || materialRequestHash.Length != 64
            || reason is { Length: > 500 }
        ...
        await using var db = await relationshipFactory.CreateDbContextAsync(cancellationToken);
        var replay = await db
            .PerformanceReviewResponses.AsNoTracking()
            .SingleOrDefaultAsync(
                value =>
                    value.TenantId == tenantId
                    && value.RelationshipId == relationshipId
        ...
        {
            if (
                replay.ReviewId != reviewId
                || replay.ReviewRevision != expectedReviewRevision
        ...
        var review =
            await db
                .PerformanceReviewWindows.AsNoTracking()
                .SingleOrDefaultAsync(
                    value =>
                        value.TenantId == tenantId
                        && value.RelationshipId == relationshipId
        ...
        if (relationship.State == EmploymentRelationshipState.StoppedEmergency)
            throw new ConstitutionalActionDeniedException(
                "Performance review response is blocked by Emergency Stop."
            );
        var normalizedReason = reason?.Trim();
        var reasonHash = normalizedReason is null
            ? null
        ...
        var evidenceId = await constitutionalGateway.AuthorizeAndRecordAsync(
            ...
            "RELATIONSHIP_PERFORMANCE_REVIEW_RESPONSE",
            correlationId,
            new
            {
                actorParticipantId,
        ...
        var responseRevision =
            await db
                .PerformanceReviewResponses.Where(value =>
                    value.TenantId == tenantId
                    && value.RelationshipId == relationshipId
                    && value.ReviewId == reviewId
                )
                .Select(value => (int?)value.ResponseRevision)
                .MaxAsync(cancellationToken)
            ?? 0;
    ...
    )
    {
        ...
        return reviews
            .Select(review =>
                Project(
                    review,
                    responses.FirstOrDefault(response => response.ReviewId == review.ReviewId)
                )
            )
            .ToArray();
    }
    ...
    )
    {
        if (
            ...
            || string.IsNullOrWhiteSpace(draft.PolicyVersion)
            || draft.PeriodStart >= draft.PeriodEnd
            || draft.SourceVersions.Count == 0
            || !Recommendations.Contains(draft.Recommendation)
            || draft.EvidenceId == Guid.Empty
        )
            throw new PerformanceReviewInvalidException();
        foreach (
            var dimension in new[]
            {
                draft.WorkDelivery,
                draft.AgentQuality,
                draft.ConstitutionalPerformance,
                draft.CommercialUsage,
                draft.CustomerBusinessOutcome,
                draft.CustomerAssessment,
                draft.TrustAutonomy,
            }
        )
            if (
                string.IsNullOrWhiteSpace(dimension.State)
    ...
    private static PerformanceReviewProjectionV1 Project(
        ...
        new(
            ...
            JsonSerializer.Deserialize<Dictionary<string, string>>(
                value.SourceVersionsJson,
                JsonOptions
            )!,
            Deserialize(value.WorkDeliveryJson),
            Deserialize(value.AgentQualityJson),
            Deserialize(value.ConstitutionalPerformanceJson),
            Deserialize(value.CommercialUsageJson),
            Deserialize(value.CustomerBusinessOutcomeJson),
            Deserialize(value.CustomerAssessmentJson),
            Deserialize(value.TrustAutonomyJson),
            ...
            value.Recommendation is "REASSESSMENT_REQUIRED" or "PAUSE_AFFECTED_WORK"
                || response?.Decision
                    is "REQUEST_REASSESSMENT"
                        or "DISPUTE_ASSESSMENT"
                        or "PAUSE_AFFECTED_WORK"
        );

    private static PerformanceReviewResponseV1 Project(PerformanceReviewResponse value) =>
        new(
            value.ResponseId,
            value.ReviewId,
