...
public sealed class IdentityBrokerReadOptions
{
    ...
    public bool IsConfigured => Enabled
        ...
        && issuer.Scheme == "https" && issuer.AbsolutePath == "/realms/waooaw"
        && issuer.Query == "" && issuer.Fragment == "" && issuer.UserInfo == ""
        && Uri.TryCreate(PrivateOrigin, UriKind.Absolute, out var origin)
        && origin.Scheme == "https" && origin.AbsolutePath == "/"
        ...
        && Providers.Count > 0 && Providers.All(provider => provider.Key is "google" or "facebook" or "apple"
...
public sealed class GoogleWorkspaceProofAdapter(HttpClient client, IOptions<IdentityBrokerReadOptions> options)
{
    ...
    public VerifiedCustomerActor ValidateActor(ClaimsPrincipal principal, bool requireFresh = false)
    {
        ...
        if (principal.Identity?.IsAuthenticated != true || issuer != _options.ActorIssuer
            || !ValidKey(subject) || subject is "." or ".." || subject!.StartsWith("service-account-", StringComparison.Ordinal)
            || !_options.AllowedAuthorizedParties.Contains(SingleClaim(principal, "azp"), StringComparer.Ordinal)
            ...
            || expires <= issued || expires - issued > 900
            || !Timestamp(principal, "auth_time", out var authenticated) || authenticated > now + 30
    ...
    public async Task<VerifiedGoogleWorkspaceProof> ReadAsync(ClaimsPrincipal principal, CancellationToken ct)
    {
        ...
        try
        {
            using var tokenRequest = new HttpRequestMessage(HttpMethod.Post,
                new Uri(new Uri(_options.PrivateOrigin), "/realms/waooaw/protocol/openid-connect/token"))
            {
            ...
            if (token.RootElement.GetProperty("token_type").GetString() != "Bearer"
            ...
            if (root.GetProperty("id").GetString() != actor.Subject
                ...
                || root.TryGetProperty("username", out var username)
                    && username.GetString()?.StartsWith("service-account-", StringComparison.Ordinal) == true)
                throw new IdentityActionDeniedException("IDENTITY_ACTION_DENIED");
            using var bindings = await ReadUserAsync(path + "/federated-identity", accessToken, timeout.Token);
            if (bindings.RootElement.ValueKind != JsonValueKind.Array || bindings.RootElement.GetArrayLength() != 1)
                throw new IdentityActionDeniedException("IDENTITY_ACTION_DENIED");
    ...
    {
        using var request = new HttpRequestMessage(HttpMethod.Get, new Uri(new Uri(_options.PrivateOrigin), path));
    ...
    private async Task<JsonDocument> SendAsync(HttpRequestMessage request, CancellationToken ct)
    {
        ...
        var document = JsonDocument.Parse(buffer.ToArray(), new JsonDocumentOptions { MaxDepth = 16 });
    ...
    internal static bool ValidKey(string? value) => !string.IsNullOrWhiteSpace(value)
        && Encoding.UTF8.GetByteCount(value) <= 256 && !value.Any(char.IsControl)
    ...
    internal static string? SingleClaim(ClaimsPrincipal principal, string type)
    {
        ...
        if (claims.Length == 0 && JwtSecurityTokenHandler.DefaultInboundClaimTypeMap.TryGetValue(type, out var mapped))
    ...
    private static bool Timestamp(ClaimsPrincipal principal, string type, out long value) =>
        long.TryParse(SingleClaim(principal, type), out value) && value > 0 && value <= 253402300799;
    ...
    private static bool CustomerRolesOnly(ClaimsPrincipal principal)
    {
        try
        {
            ...
            var roles = realm.RootElement.GetProperty("roles").EnumerateArray().Select(role => role.GetString()).ToArray();
            return roles.Contains("customer", StringComparer.Ordinal)
                && roles.All(role => role is "customer" or "offline_access" or "uma_authorization" or "default-roles-waooaw")
                && !principal.FindAll(ClaimTypes.Role).Concat(principal.FindAll("role"))
                    .Any(role => role.Value is not ("customer" or "offline_access" or "uma_authorization" or "default-roles-waooaw"));
        }
