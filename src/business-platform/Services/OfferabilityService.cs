...
public sealed class OfferabilityService
{
    public OfferabilityDecision Evaluate(OfferabilityInput input)
    {
        ...
        if (
            input.OwnerVersions.Count == 0
            || input.OwnerVersions.Any(value => string.IsNullOrWhiteSpace(value.Value))
        )
        ...
        var disposition =
            reasons.Count == 0 ? OfferabilityDisposition.Allow
            : reasons.Contains("CONSTITUTIONAL_FLOOR_FAILED")
            || reasons.Contains("CUSTOMER_PROTECTION_FAILED")
                ? OfferabilityDisposition.Block
            : reasons.Contains("CALCULATED_RISK_DISABLED")
            || reasons.Contains("OWNER_EVIDENCE_CONFLICTING")
    ...
    )
    {
        ...
        if (
            ...
            throw new ActivationEligibilityException(
                "Current eligible offerability decision is required."
            );

        var professionalType = await db
            .EmploymentRelationships.AsNoTracking()
            .Where(value => value.TenantId == tenantId && value.RelationshipId == relationshipId)
            .Select(value => value.ProfessionalType)
            .SingleAsync(cancellationToken);
        var admitted = await db
        ...
        if (admitted.Count != 1)
            throw new ActivationEligibilityException(
                "Exactly one active admitted professional version is required."
            );
        ...
        var revision =
            ...
            ?? throw new ActivationEligibilityException(
                "Current admission revision is unavailable."
            );
        using var document = JsonDocument.Parse(revision.AdmissionContentJson);
        var exemptSkills = document
            .RootElement.GetProperty("skillManifest")
            .EnumerateArray()
        ...
        if (
            ...
            || configurations.Any(value =>
                value.GoalId is null && !exemptSkills.Contains(value.SkillId)
            )
        )
            throw new ActivationEligibilityException(
                "Every activated skill requires a goal or admitted non-goal exemption."
            );
    }
}
