...
public sealed record PaymentProceedRequest(
    string BundleTier,
    long SubscriptionAmountInrPaise,
    long WalletSeedInrPaise,
    string ProceedConfirmation
);
...
public sealed record RelationshipCheckoutOutcome(
    ...
    string? OrderId = null,
    string? PublicCheckoutKey = null,
    long? AmountInrPaise = null,
    string Currency = "INR",
    string? QuoteVersion = null,
    string? CheckoutSessionId = null,
    string? ProviderOrderReference = null,
    string? MerchantDisplayName = null,
    IReadOnlyList<string>? EnabledMethodFamilies = null,
    ...
    long? PayableInrPaise = null,
    string? RenewalConsequence = null,
    string? CommercialOutcomeReference = null,
    Guid? CommercialEvidenceId = null,
    string? EvidenceState = null,
    string? ReasonCode = null,
    string? AccountableOwner = null,
    bool? Retryable = null,
    string? CustomerSafeNextAction = null
);

public interface IRelationshipPaymentGateway
{
    Task<HostedOnboardingOrder> CreateOrderAsync(
...
)
{
    ...
    )
    {
        ...
        await RequireActiveParticipantAsync(
            db,
            tenantId,
            relationshipId,
            participantId,
        ...
        var intent = await db
            .RelationshipCheckoutIntents.AsNoTracking()
            .Where(item =>
    ...
    )
    {
        ...
        var intent =
            await db.RelationshipCheckoutIntents.SingleOrDefaultAsync(
                item =>
                    item.TenantId == tenantId
                    && item.RelationshipId == relationshipId
                    && item.CheckoutIntentId == checkoutIntentId,
                cancellationToken
            ) ?? throw new KeyNotFoundException("Checkout intent not found.");
        var stored = ToStoredOutcome(intent);
        if (intent.Status != "AWAITING_PROVIDER")
            return stored;
        ...
        var reconciled = await paymentGateway.ReconcileCheckoutAsync(
            new ContractLinkedCheckoutReconciliation(
                ...
                intent.ContractHash,
                intent.ContractAcceptanceId,
                intent.PaymentConsentEvidenceId.Value
            ),
            cancellationToken
        );
        if (
            reconciled.CheckoutIntentId != intent.CheckoutIntentId
            ...
            throw new PaymentOwnerUnavailableException(
                "WBE reconciliation identity is inconsistent with BP."
            );
        ...
        {
            throw new PaymentOwnerUnavailableException(
                "WBE reconciliation returned an invalid outcome transition."
            );
        }
    ...
    public async Task<RelationshipCheckoutOutcome> CreateCheckoutAsync(
        ...
        int contractVersion,
        Guid idempotencyKey,
        CheckoutProceedRequest request,
    ...
    )
    {
        ...
        var contract = await db
            ...
            .SingleAsync(
                item =>
                    item.TenantId == tenantId
                    && item.RelationshipId == relationshipId
                    && item.ContractId == acceptance.ContractId
                    && item.Version == acceptance.ContractVersion
                    && item.ContractHash == acceptance.ContractHash,
                cancellationToken
            );
        var terms =
            JsonSerializer.Deserialize<EmploymentContractCommercialTerms>(
                contract.PriceTaxSummaryJson,
                JsonOptions
            )
            ?? throw new PaymentOwnerUnavailableException("Accepted commercial terms are invalid.");
        var materialHash = Hash(
            string.Join(
                '|',
                tenantId,
                relationshipId,
        ...
        if (existing is not null)
        {
            ...
            if (existing.OutcomeJson is not null)
                return JsonSerializer.Deserialize<RelationshipCheckoutOutcome>(
                        existing.OutcomeJson,
                        JsonOptions
                    )
                    ?? throw new PaymentOwnerUnavailableException(
                        "Stored checkout outcome is invalid."
                    );
        ...
        try
        {
            outcome = await paymentGateway.CreateCheckoutAsync(
                new ContractLinkedCheckoutRequest(
                    intent.CheckoutIntentId,
                    tenantId,
                    relationship.InitiatingParticipantId,
                    relationshipId,
                    contract.ContractId,
                    contract.Version,
                    contract.ContractHash,
        ...
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            ...
            intent.OutcomeJson = JsonSerializer.Serialize(
                UnresolvedOutcome(intent.CheckoutIntentId, relationshipId, contractVersion),
                JsonOptions
            );
        ...
        if (
            outcome.CheckoutIntentId != intent.CheckoutIntentId
            || outcome.RelationshipId != relationshipId
            || outcome.ContractVersion != contractVersion
        )
            throw new PaymentOwnerUnavailableException(
                "WBE returned checkout identity inconsistent with BP."
            );
    ...
    )
    {
        ...
        if (
            ...
            || authenticationAge > TimeSpan.FromMinutes(5)
        ...
        if (!employer)
            throw new ConstitutionalActionDeniedException(
                "Payment initiation requires an active same-tenant EMPLOYER binding."
            );
        var acceptance =
            await db
                .ContractAcceptances.AsNoTracking()
        ...
        var order = await paymentGateway.CreateOrderAsync(
            new ContractLinkedOnboardingOrderRequest(
                tenantId,
                relationship.InitiatingParticipantId,
                relationshipId,
                contract.ContractId,
                ...
                evidenceId,
                relationship.ProfessionalType,
                request.BundleTier.Trim().ToUpperInvariant(),
                request.SubscriptionAmountInrPaise,
                request.WalletSeedInrPaise
            ),
            cancellationToken
        );
        if (order.IsBypass || order.Currency != "INR" || order.AmountInrPaise != grossAmount)
            throw new PaymentOwnerUnavailableException(
                "WBE returned an order inconsistent with the accepted contract."
            );
        return order;
    }
    ...
    private static RelationshipCheckoutOutcome ToStoredOutcome(RelationshipCheckoutIntent intent) =>
        intent.OutcomeJson is null
            ...
            : JsonSerializer.Deserialize<RelationshipCheckoutOutcome>(
                intent.OutcomeJson,
                JsonOptions
...
public sealed class HttpRelationshipPaymentGateway(IHttpClientFactory httpClientFactory)
    : IRelationshipPaymentGateway
{
    ...
    )
    {
        using var response = await httpClientFactory
            .CreateClient("WBE")
            .PostAsJsonAsync(
                $"/payments/relationship-checkout/{request.CheckoutIntentId:D}/reconcile",
                request,
                JsonOptions,
                cancellationToken
            );
        ...
        return await response.Content.ReadFromJsonAsync<RelationshipCheckoutOutcome>(
                JsonOptions,
                cancellationToken
            )
            ?? throw new PaymentOwnerUnavailableException(
                "WBE returned an empty reconciliation outcome."
            );
    }
    ...
    )
    {
        using var response = await httpClientFactory
            .CreateClient("WBE")
            .PostAsJsonAsync(
                "/payments/relationship-checkout",
                request,
                JsonOptions,
                cancellationToken
            );
        ...
        return await response.Content.ReadFromJsonAsync<RelationshipCheckoutOutcome>(
            ...
            ?? throw new PaymentOwnerUnavailableException(
                "WBE returned an empty checkout outcome."
            );
    }
    ...
    )
    {
        using var response = await httpClientFactory
            ...
            .PostAsJsonAsync(
                ...
                new WbeOrderRequest(
                    ...
                    request.PaymentConsentEvidenceId,
                    request.AgentType,
                    request.BundleTier,
                    request.SubscriptionAmountInrPaise,
                    request.WalletSeedInrPaise
        ...
        if (!response.IsSuccessStatusCode)
            throw new PaymentOwnerUnavailableException(
                $"WBE onboarding order returned {(int)response.StatusCode}."
            );
        var result =
            await response.Content.ReadFromJsonAsync<WbeOrderResult>(cancellationToken)
            ?? throw new PaymentOwnerUnavailableException(
                "WBE returned an empty onboarding order."
            );
        return new HostedOnboardingOrder(
            result.OrderId,
            result.AmountPaise,
            result.Currency,
            result.IsBypass
