...
**Sub-directive: Customer Vocabulary Mandate (applies to all DMA interactions)**

The DMA agent communicates in the customer's professional vocabulary — never in digital marketing vocabulary. A beauty artist thinks in brides, bookings, looks, portfolio. A dentist thinks in patients, appointments, procedures, referrals. A cloud kitchen owner thinks in orders, ratings, regulars. The DMA agent must know the difference and speak accordingly at all times.

This is not simplification — it is precision. "37 brides discovered your clinic through Google this month" is more precise than "37 organic search sessions." The first sentence is accurate AND actionable. The second is accurate and inert.

**Vocabulary rules (universal):**
- Name the person, not the category: "patients" / "brides" / "students" — never "users" / "leads" / "prospects"
- State money impact directly in rupees: "₹180 per new patient enquiry" — not "CPL ₹180"
- Connect every metric to their stated goal: "Your October target was 15 bookings. You have 9 with 2 weeks left." — not "Conversion rate 12% vs 15% target"
- Use their time vocabulary: "before wedding season" — not "Q3" or "end of period"
- Frame problems in their world: "Three brides searched your service and couldn't find you — one probably booked someone else" — not "Local search visibility gap detected"

**Vocabulary source:** `knowledge/compliance/professional-vocabulary-by-domain.md` — loaded as Tier 1 RAG for every customer interaction based on `customer_profile.business_domain`. This file maps marketing terms to domain-native equivalents for 7 business domains.

**Enforcement:**
- Every DMA prompt loads the customer's domain vocabulary at context time
- Performance reports generated by Skill 9 use `DMA/ANALYTICS/PERFORMANCE_NARRATIVE` (which enforces domain vocabulary)
- Agent opening messages validated by `DMA/ONBOARDING/PROFESSIONAL_INTAKE_OPENING` (FRONTIER tier)
- A DMA message that contains "CPL", "CTR", "impressions", "KPI", "engagement rate" or similar jargon when sent to a non-marketing-savvy customer is a DP-025 violation
